#' Fetch NHL scoreboard data using Site API
#'
...
#'
#' @examples
#' # Get NHL scoreboard for current date
#' fetch_nhl_scoreboard()
#' head(nhl_scoreboard)
#'
#' # Get scoreboard for specific date
#' fetch_nhl_scoreboard(dates = "20241225")
#'
#' # Get playoff games
#' fetch_nhl_scoreboard(seasontype = 3)
#'
#' # Get raw data
#' fetch_nhl_scoreboard(raw = TRUE)
#'
fetch_nhl_scoreboard <- function(year = NULL, week = NULL, seasontype = 2, dates = NULL, raw = FALSE) {
  # Validate inputs
  if (!is.null(year) && (!is.numeric(year) || year < 1917 || year > as.numeric(format(Sys.Date(), "%Y")) + 2)) {
    stop("year must be a valid numeric year between 1917 and two years in the future")
  }
  ...
  # Fetch and parse
  tryCatch({
    message("Fetching NHL scoreboard data...")
    ...
    if (isTRUE(raw)) {
      assign("nhl_scoreboard_raw", data, envir = .GlobalEnv)
      message("Raw NHL scoreboard data assigned to: nhl_scoreboard_raw")
      return(invisible(data))
    }

    # Create clean scoreboard dataset
    scoreboard_df <- create_clean_nhl_scoreboard_dataset(data, year, week, seasontype, dates)

    # Assign to global environment
    assign("nhl_scoreboard", scoreboard_df, envir = .GlobalEnv)
    message(sprintf("NHL scoreboard data assigned to: nhl_scoreboard (%d games)", nrow(scoreboard_df)))

    return(invisible(scoreboard_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch NHL scoreboard data: %s", e$message))
  })
}

#' Create clean NHL scoreboard dataset from ESPN Site API response
#'
...
create_clean_nhl_scoreboard_dataset <- function(data, year, week, seasontype, dates) {
  ...
  for (i in seq_along(events)) {
    ...
    if ("competitions" %in% names(event) && length(event[["competitions"]]) > 0) {
      ...
      # Clock and period info (hockey-specific)
      period <- if ("period" %in% names(status_info)) as.integer(status_info[["period"]]) else NA_integer_
      ...
      if (is_completed && !is.na(status_detail)) {
      ...
      if ("broadcasts" %in% names(competition) && length(competition[["broadcasts"]]) > 0) {
        ...
        if ("names" %in% names(first_broadcast) && length(first_broadcast[["names"]]) > 0) {
          broadcast <- paste(first_broadcast[["names"]], collapse = ", ")
        }
      }
      ...
      home_goals <- away_goals <- NA_integer_
      ...
      if ("competitors" %in% names(competition) && length(competition[["competitors"]]) > 0) {
        for (j in seq_along(competition[["competitors"]])) {
          ...
          goals <- if (!is.na(score) && score != "") as.integer(score) else NA_integer_
