#' Fetch NHL team injuries data using Core API
#'
#' Retrieves current injury information for a specific NHL team from ESPN's Core API.
#' The function fetches detailed injury reports including player status, injury type,
#' and recovery timeline information.
#'
#' @param team_id Character or Numeric. ESPN NHL team ID (required).
#'   Examples: "4" for Boston Bruins, "3" for New York Rangers, "6" for Detroit Red Wings.
#'   See ESPN's NHL team directory for complete team ID list.
#' @param raw Logical. If TRUE, assigns raw JSON response to global environment
#'   as 'nhl_injuries_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns the processed injury data frame. The main purpose is global
#'   environment assignment of:
#'   \itemize{
#'     \item \code{nhl_team_injuries} - Clean injury data frame with player details
#'     \item \code{nhl_injuries_raw} - Raw JSON response (if raw = TRUE)
#'   }
#'
#' @details
...
#' @examples
#' \dontrun{
#' # Get current injuries for Boston Bruins
#' fetch_nhl_team_injuries(team_id = "4")
#' head(nhl_team_injuries)
#'
#' # Get New York Rangers injuries
#' fetch_nhl_team_injuries(team_id = "3")
#'
#' # View injury types and statuses
#' table(nhl_team_injuries$injury_status)
#' table(nhl_team_injuries$injury_type_name)
#'
#' # Get raw data for custom processing
#' fetch_nhl_team_injuries(team_id = "4", raw = TRUE)
#' str(nhl_injuries_raw, max.level = 2)
#'
#' # Analyze current active injuries
#' active_injuries <- nhl_team_injuries[
#'   nhl_team_injuries$injury_status %in% c("Out", "Day-to-Day", "Injured Reserve"),
...
#' @export
fetch_nhl_team_injuries <- function(team_id, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL for NHL Core API
  url <- sprintf("https://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/teams/%s/injuries", team_id)

  message(sprintf("Fetching NHL injuries for team ID: %s...", team_id))
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("nhl_injuries_raw", data, envir = .GlobalEnv)
      message("Raw NHL injuries data assigned to: nhl_injuries_raw")
    ...
    injuries_df <- create_clean_nhl_team_injuries_dataset(data, team_id)

    # Assign to global environment
    assign("nhl_team_injuries", injuries_df, envir = .GlobalEnv)
    message(sprintf("NHL team injuries assigned to: nhl_team_injuries (Team ID: %s, %d injuries)",
    ...
    stop(sprintf("Failed to fetch NHL injuries for team %s: %s", team_id, e$message))
  })
}

#' Create clean NHL team injuries dataset from ESPN Core API response
#'
...
#' @keywords internal
create_clean_nhl_team_injuries_dataset <- function(data, team_id) {
  ...
  if (nrow(result_df) > 0) {
    ...
    # Define priority order for injury statuses (NHL-specific)
    status_priority <- c("Out", "Injured Reserve", "Long-term Injured Reserve", "Day-to-Day",
...
#' Fetch multiple NHL team injuries data
#'
#' Retrieves injury information for multiple NHL teams with rate limiting to
#' be respectful to ESPN's Core API. This function calls \code{\link{fetch_nhl_team_injuries}}
#' for each team and combines the results.
#'
#' @param team_ids Character or Numeric vector. ESPN NHL team IDs.
...
#' fantasy hockey research, or medical staff benchmarking studies.
#'
#' @examples
#' \dontrun{
#' # Get injuries for multiple teams
#' original_six <- c("4", "3", "8", "10", "16", "17")  # Bruins, Rangers, Canadiens, Maple Leafs, Blackhawks, Red Wings
#' fetch_multiple_nhl_team_injuries(original_six)
#'
#' # Use longer delay for larger requests
#' eastern_conference <- c("4", "7", "12", "13", "1", "2", "5", "9")
#' fetch_multiple_nhl_team_injuries(eastern_conference, delay = 0.5)
#'
#' # Analyze combined results
#' unique_teams <- unique(nhl_team_injuries$request_team_id)
#' cat("Retrieved injury data for", length(unique_teams), "teams\n")
#'
#' # League-wide injury analysis
#' injury_summary <- table(nhl_team_injuries$injury_type_name)
#' print(injury_summary)
#'
#' # Teams with most injuries
#' team_injury_counts <- table(nhl_team_injuries$request_team_id)
#' print(sort(team_injury_counts, decreasing = TRUE))
#' }
#'
#' @importFrom httr GET status_code content timeout
#' @export
fetch_multiple_nhl_team_injuries <- function(team_ids, delay = 0.3, raw = FALSE) {
  # Input validation
  ...
  message(sprintf("Starting to fetch NHL injury data for %d teams...", length(team_ids)))

  # Process each team sequentially
  for (i in seq_along(team_ids)) {
    team_id <- team_ids[i]
    message(sprintf("Fetching NHL injuries for team %s (%d/%d)...", team_id, i, length(team_ids)))

    tryCatch({
      # Fetch individual team data
      team_data <- fetch_nhl_team_injuries(
        team_id = team_id,
        raw = raw
      )
      ...
      # Combine data
      injuries_df <- get("nhl_team_injuries", envir = .GlobalEnv)
      all_injuries <- rbind(all_injuries, injuries_df)

    }, error = function(e) {
      message(sprintf("Failed to fetch NHL injuries for team %s: %s", team_id, e$message))
    })
  ...
  if (nrow(all_injuries) > 0) {
    ...
    assign("nhl_team_injuries", all_injuries, envir = .GlobalEnv)

    unique_teams <- length(unique(all_injuries$request_team_id))
    total_injuries <- nrow(all_injuries)

    message(sprintf("Combined NHL team injuries assigned to: nhl_team_injuries (%d teams, %d injuries)",
