#' Fetch MLB team schedule data using ESPN Site API
#'
...
#' @param raw Logical. If TRUE, assigns raw JSON to global environment as 'mlb_schedule_raw'. Default FALSE.
...
#' fetch_mlb_team_schedule(team_id = "10")
#' head(mlb_team_schedule)
#'
#' # Get 2023 season schedule
#' fetch_mlb_team_schedule(team_id = 10, season = 2023)
#'
#' # Get raw data for custom processing
#' fetch_mlb_team_schedule(team_id = "10", season = 2024, raw = TRUE)
#' str(mlb_schedule_raw, max.level = 2)
#'
fetch_mlb_team_schedule <- function(team_id, season = NULL, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL
  base_url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/teams/%s/schedule", team_id)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("mlb_schedule_raw", data, envir = .GlobalEnv)
      message("Raw MLB schedule data assigned to: mlb_schedule_raw")
      return(invisible(data))
    }

    # Create clean schedule dataset
    schedule_df <- create_clean_mlb_schedule_dataset(data, team_id, season)

    # Assign to global environment
    assign("mlb_team_schedule", schedule_df, envir = .GlobalEnv)
    season_text <- if (!is.null(season)) season else "current"
    message(sprintf("MLB team schedule assigned to: mlb_team_schedule (Team ID: %s, %s season, %d games)",
                    team_id, season_text, nrow(schedule_df)))

    return(invisible(schedule_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch MLB schedule for team %s: %s", team_id, e$message))
  })
}

#' Create clean MLB team schedule dataset from ESPN Site API response
#'
...
create_clean_mlb_schedule_dataset <- function(data, team_id, season) {
