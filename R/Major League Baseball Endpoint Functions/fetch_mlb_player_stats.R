#' Safe nested data extraction helper function for MLB player statistics
#'
...
#' @keywords internal
extract_nested_mlb_player_stats <- function(data, path, default = NA) {
  tryCatch({
    result <- data
    for (key in path) {
...
#' Get MLB player information from ESPN player profile API
#'
...
#' @keywords internal
fetch_mlb_player_info <- function(player_id) {
  # Try the player profile endpoint
  url <- paste0("https://site.web.api.espn.com/apis/common/v3/sports/baseball/mlb/athletes/", player_id)
  ...
  tryCatch({
    ...
    if (httr::status_code(resp) == 200) {
      ...
      athlete <- extract_nested_mlb_player_stats(player_data, c("athlete"), list())
      if (length(athlete) > 0) {
        return(list(
          name = extract_nested_mlb_player_stats(athlete, c("name"), NA_character_),
          display_name = extract_nested_mlb_player_stats(athlete, c("displayName"), NA_character_),
          jersey = extract_nested_mlb_player_stats(athlete, c("jersey"), NA_character_),
          position = extract_nested_mlb_player_stats(athlete, c("position", "abbreviation"), NA_character_),
          team_id = extract_nested_mlb_player_stats(athlete, c("team", "id"), NA_character_),
          team_abbreviation = extract_nested_mlb_player_stats(athlete, c("team", "abbreviation"), NA_character_),
          team_display_name = extract_nested_mlb_player_stats(athlete, c("team", "displayName"), NA_character_),
          bats = extract_nested_mlb_player_stats(athlete, c("bats", "abbreviation"), NA_character_),
          throws = extract_nested_mlb_player_stats(athlete, c("throws", "abbreviation"), NA_character_)
        ))
      }
    }
  }, error = function(e) {
...
#' Parse ESPN MLB statistics format
#'
...
#' @keywords internal
parse_mlb_espn_stats <- function(stats_vector, stat_type = "batting") {
...
#' Create MLB season summary statistics data frame from ESPN API
#'
...
#' @keywords internal
create_mlb_season_summary_dataset <- function(data, player_id, player_info) {
  ...
  season_types <- extract_nested_mlb_player_stats(data, c("seasonTypes"), NULL)
        ...
        for (j in 1:nrow(summary_for_season)) {
          ...
          if ("stats" %in% names(summary_for_season) && length(summary_for_season$stats) >= j) {
            ...
            if (!is.null(stats_data) && length(stats_data) > 0) {
              ...
              if (is_batting) {
                parsed_stats <- parse_mlb_espn_stats(stats_data, "batting")
                ...
                parsed_stats <- parse_mlb_espn_stats(stats_data, "pitching")
...
#' Create detailed MLB player game log data frame from ESPN API
#'
...
#' @keywords internal
create_mlb_player_gamelog_dataset <- function(data, player_id, player_info) {
  ...
  events_data <- extract_nested_mlb_player_stats(data, c("events"), NULL)
  ...
  for (event_name in names(events_data)) {
    ...
    if (!is.null(event_data) && is.list(event_data)) {
      ...
      game_id <- extract_nested_mlb_player_stats(event_data, c("id"), event_name)
      game_date <- extract_nested_mlb_player_stats(event_data, c("gameDate"), NA_character_)
      at_vs <- extract_nested_mlb_player_stats(event_data, c("atVs"), NA_character_)
      score <- extract_nested_mlb_player_stats(event_data, c("score"), NA_character_)
      result <- extract_nested_mlb_player_stats(event_data, c("gameResult"), NA_character_)

      # Extract opponent information
      opponent <- extract_nested_mlb_player_stats(event_data, c("opponent"), list())
      opponent_id <- extract_nested_mlb_player_stats(opponent, c("id"), NA_character_)
      opponent_abbreviation <- extract_nested_mlb_player_stats(opponent, c("abbreviation"), NA_character_)

      # Extract team information
      team <- extract_nested_mlb_player_stats(event_data, c("team"), list())
      team_abbreviation <- extract_nested_mlb_player_stats(team, c("abbreviation"), NA_character_)
...
#' Fetch detailed MLB player statistics using ESPN Web API
#'
#' Retrieves comprehensive statistics for a specific MLB player from ESPN's Web API.
#' Extracts data directly from the seasonTypes structure - fast and reliable!
#'
#' @param player_id Character or numeric. MLB player ID from ESPN
...
#'   as 'mlb_player_stats_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns a list with both datasets. The main purpose is global
#'   environment assignment of:
#'   \itemize{
#'     \item \code{mlb_player_season_summary}: Season averages and totals by season type
#'     \item \code{mlb_player_gamelog}: Game-by-game statistics organized by category
#'   }
#'
#' @details
...
#' fetch_mlb_player_stats("30951")
#'
#' # Check season summaries
#' print(mlb_player_season_summary)
#'
#' # Check game log
#' head(mlb_player_gamelog)
#'
#' # View regular season batting averages
#' reg_season_batting <- mlb_player_season_summary[
#'   mlb_player_season_summary$season_display_name == "2024 Regular Season" &
#'   mlb_player_season_summary$stat_category == "batting" &
#'   mlb_player_season_summary$stat_type == "avg", ]
#' print(reg_season_batting[, c("avg", "home_runs", "rbi", "ops")])
#'
#' # View recent games
#' recent_games <- head(mlb_player_gamelog[
#'   mlb_player_gamelog$stat_category == "batting" &
#'   !is.na(mlb_player_gamelog$at_bats), ], 10)
#' print(recent_games[, c("category_display_name", "at_bats", "hits", "home_runs", "rbi")])
#'
#' # View pitching statistics (for pitchers)
#' pitching_stats <- mlb_player_season_summary[
#'   mlb_player_season_summary$stat_category == "pitching" &
#'   mlb_player_season_summary$stat_type == "total", ]
#' print(pitching_stats[, c("era", "whip", "innings_pitched", "strikeouts_pitching", "wins")])
#'
#' # Calculate batting average from game log
#' batting_games <- mlb_player_gamelog[
#'   mlb_player_gamelog$stat_category == "batting" &
#'   !is.na(mlb_player_gamelog$at_bats) &
#'   as.numeric(mlb_player_gamelog$at_bats) > 0, ]
#'
...
#' @export
fetch_mlb_player_stats <- function(player_id, raw = FALSE) {
  # Input validation
  ...
  player_info <- fetch_mlb_player_info(player_id)
  ...
  url <- paste0("https://site.web.api.espn.com/apis/common/v3/sports/baseball/mlb/athletes/",
                player_id, "/gamelog")

  message("Fetching MLB player statistics...")
  ...
  if (isTRUE(raw)) {
    assign("mlb_player_stats_raw", data, envir = .GlobalEnv)
    message("Raw MLB player statistics data assigned to: mlb_player_stats_raw")
  ...
  gamelog <- create_mlb_player_gamelog_dataset(data, player_id, player_info)
  assign("mlb_player_gamelog", gamelog, envir = .GlobalEnv)
...
#' Fetch multiple MLB player statistics
#'
#' Retrieves statistics for multiple MLB players with rate limiting to
#' be respectful to ESPN's API. This function calls \code{\link{fetch_mlb_player_stats}}
#' for each player and combines the results.
#'
#' @param player_ids Character or Numeric vector. ESPN MLB player IDs.
...
#' fetch_multiple_mlb_player_stats(player_ids)
#'
#' # Use longer delay for larger requests
#' fetch_multiple_mlb_player_stats(player_ids, delay = 1.0)
#'
#' # Analyze combined results
#' unique_players <- unique(mlb_player_season_summary$player_display_name)
#' cat("Retrieved statistics for", length(unique_players), "players\n")
#'
#' # Compare batting averages
#' batting_comparison <- mlb_player_season_summary[
#'   mlb_player_season_summary$stat_category == "batting" &
#'   mlb_player_season_summary$stat_type == "avg",
...
#' }
#'
#' @seealso \code{\link{fetch_mlb_player_stats}} for single player data
#' @importFrom httr GET status_code content timeout
#' @export
fetch_multiple_mlb_player_stats <- function(player_ids, delay = 0.5, raw = FALSE) {
  # Input validation
  ...
  message(sprintf("Starting to fetch MLB player statistics for %d players...", length(player_ids)))

  # Process each player sequentially
  for (i in seq_along(player_ids)) {
    player_id <- player_ids[i]
    message(sprintf("Fetching MLB player stats for player %s (%d/%d)...", player_id, i, length(player_ids)))

    tryCatch({
      # Fetch individual player data
      player_data <- fetch_mlb_player_stats(
        player_id = player_id,
        raw = raw
      )
      ...
      # Combine data
      summary_df <- get("mlb_player_season_summary", envir = .GlobalEnv)
      gamelog_df <- get("mlb_player_gamelog", envir = .GlobalEnv)

      all_season_summary <- rbind(all_season_summary, summary_df)
      all_gamelog <- rbind(all_gamelog, gamelog_df)

    }, error = function(e) {
      message(sprintf("Failed to fetch MLB player stats for player %s: %s", player_id, e$message))
    })
  ...
  if (nrow(all_season_summary) > 0) {
    ...
    assign("mlb_player_season_summary", all_season_summary, envir = .GlobalEnv)

    unique_players <- length(unique(all_season_summary$player_id))
    message(sprintf("Combined MLB player season summary assigned to: mlb_player_season_summary (%d players, %d records)",
  ...
  if (nrow(all_gamelog) > 0) {
    ...
    assign("mlb_player_gamelog", all_gamelog, envir = .GlobalEnv)

    unique_players <- length(unique(all_gamelog$player_id))
    total_games <- nrow(all_gamelog)
    message(sprintf("Combined MLB player game log assigned to: mlb_player_gamelog (%d players, %d games)",
