#' Fetch mlb scoreboard data using Site API
#'
...
#'
#' @examples
#' # Get mlb scoreboard for current date
#' fetch_mlb_scoreboard()
#' head(mlb_scoreboard)
#'
#' # Get scoreboard for specific date
#' fetch_mlb_scoreboard(dates = "20241225")
#'
#' # Get raw data
#' fetch_mlb_scoreboard(raw = TRUE)
#'
fetch_mlb_scoreboard <- function(year = NULL, week = NULL, seasontype = 2, dates = NULL, raw = FALSE) {
  ...
  # Fetch and parse
  tryCatch({
    message("Fetching mlb scoreboard data...")
    ...
    if (isTRUE(raw)) {
      assign("mlb_scoreboard_raw", data, envir = .GlobalEnv)
      message("Raw mlb scoreboard data assigned to: mlb_scoreboard_raw")
      return(invisible(data))
    }

    # Create clean scoreboard dataset
    scoreboard_df <- create_clean_mlb_scoreboard_dataset(data, year, week, seasontype, dates)

    # Assign to global environment
    assign("mlb_scoreboard", scoreboard_df, envir = .GlobalEnv)
    message(sprintf("mlb scoreboard data assigned to: mlb_scoreboard (%d games)", nrow(scoreboard_df)))

    return(invisible(scoreboard_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch mlb scoreboard data: %s", e$message))
  })
}

#' Create clean mlb scoreboard dataset from ESPN Site API response
#'
...
create_clean_mlb_scoreboard_dataset <- function(data, year, week, seasontype, dates) {
