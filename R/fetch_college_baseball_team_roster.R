#' Fetch college_baseball team roster data using ESPN Site API
#'
...
#' @param raw Logical. If TRUE, assigns raw JSON to global environment as 'college_baseball_roster_raw'. Default FALSE.
#' @return Invisibly returns the data, but main purpose is global environment assignment.
#' @export
#'
#' @examples
#' # Get roster using team ID
#' fetch_college_baseball_team_roster(team_id = "10")
#'
#' # Get roster using team name
#' fetch_college_baseball_team_roster(team_id = "New York Yankees")
#'
#' # Get roster using team abbreviation
#' fetch_college_baseball_team_roster(team_id = "NYY")
#'
#' # Get raw data for custom processing
#' fetch_college_baseball_team_roster(team_id = "10", raw = TRUE)
#' str(college_baseball_roster_raw, max.level = 2)
#'
fetch_college_baseball_team_roster <- function(team_id, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  if (!grepl("^\\d+$", team_input)) {
    ...
    resolved_ids <- find_college_baseball_team_id(team_input)
  ...
  # Build URL for college_baseball
  url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/baseball/college-baseball/teams/%s/roster", team_id_final)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("college_baseball_roster_raw", data, envir = .GlobalEnv)
      message("Raw college_baseball roster data assigned to: college_baseball_roster_raw")
      return(invisible(data))
    }

    # Create clean roster dataset
    roster_df <- create_clean_college_baseball_roster_dataset(data, team_id_final)

    # Assign to global environment
    assign("college_baseball_roster", roster_df, envir = .GlobalEnv)
    message(sprintf("college_baseball roster data assigned to: college_baseball_roster (Team ID: %s, %d players)",
                    team_id_final, nrow(roster_df)))

    return(invisible(roster_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch college_baseball roster data for team %s: %s", team_input, e$message))
  })
}

#' Create clean college_baseball roster dataset from ESPN Site API response
#'
...
create_clean_college_baseball_roster_dataset <- function(data, team_id) {
  ...
  if (nrow(result_df) > 0) {
    ...
    # Define position order for college_baseball
...
#' Get college_baseball team IDs dynamically from ESPN API
#'
...
#'
#' @examples
#' # Get all college_baseball team IDs
#' team_ids <- get_college_baseball_team_ids()
#' print(team_ids)
#'
#' # Force fresh API call without caching
#' team_ids <- get_college_baseball_team_ids(cache = FALSE)
#'
get_college_baseball_team_ids <- function(cache = TRUE) {
  # Check if cached data exists and caching is enabled
  if (cache && exists(".college_baseball_teams_cache", envir = .GlobalEnv)) {
    return(get(".college_baseball_teams_cache", envir = .GlobalEnv))
  }

  # ESPN API endpoint for college_baseball teams
  url <- "https://site.api.espn.com/apis/site/v2/sports/baseball/college_baseball/teams"
  ...
  tryCatch({
    ...
    if (cache) {
      assign(".college_baseball_teams_cache", result_df, envir = .GlobalEnv)
    }

    return(result_df)

  }, error = function(e) {
    stop(sprintf("Failed to fetch college_baseball teams data: %s", e$message))
  })
}

#' Find college_baseball team ID by name or abbreviation
#'
...
#' find_college_baseball_team_id("New York Yankees")
#'
#' # Find team by abbreviation
#' find_college_baseball_team_id("NYY")
#'
#' # Find teams with partial match
#' find_college_baseball_team_id("New York")
#'
find_college_baseball_team_id <- function(team, cache = TRUE) {
  if (missing(team) || is.null(team) || team == "") {
    stop("'team' parameter is required and cannot be empty")
  }

  # Get all teams
  teams_df <- get_college_baseball_team_ids(cache = cache)
