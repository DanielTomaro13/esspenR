...
#' @param raw Logical. If TRUE, assigns raw JSON to global environment as 'womens_college_basketball_schedule_raw'. Default FALSE.
...
#'
fetch_womens_college_basketball_team_schedule <- function(team_id, season = NULL, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL
  base_url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/teams/%s/schedule", team_id)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("womens_college_basketball_schedule_raw", data, envir = .GlobalEnv)
      message("Raw womens_college_basketball schedule data assigned to: womens_college_basketball_schedule_raw")
    ...
    assign("womens_college_basketball_team_schedule", schedule_df, envir = .GlobalEnv)

    season_text <- if (!is.null(season)) season else "current"
    message(sprintf("womens_college_basketball team schedule assigned to: womens_college_basketball_team_schedule (Team ID: %s, %s season, %d games)",
                    team_id, season_text, nrow(schedule_df)))

    return(invisible(schedule_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch womens_college_basketball schedule for team %s: %s", team_id, e$message))
  })
}
