#' Fetch womens_college_basketball team roster data using Site API
#'
#' @param team_id Character or Numeric. ESPN team ID (e.g., "12" for Los Angeles Clipperss).
#' @param raw Logical. If TRUE, assigns raw JSON to global environment as 'womens_college_basketball_roster_raw'. Default FALSE.
#' @return Invisibly returns the data, but main purpose is global environment assignment.
#' @export
#'
#' @examples
#' # Get roster for Los Angeles Clipperss
#' fetch_womens_college_basketball_team_roster(team_id = "12")
#' head(womens_college_basketball_roster)
#'
#'
#' # Get raw data for custom processing
#' fetch_womens_college_basketball_team_roster(team_id = "12", raw = TRUE)
#' str(womens_college_basketball_roster_raw, max.level = 2)
#'
fetch_womens_college_basketball_team_roster <- function(team_id, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL
  url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/teams/%s/roster", team_id)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("womens_college_basketball_roster_raw", data, envir = .GlobalEnv)
      message("Raw womens_college_basketball roster data assigned to: womens_college_basketball_roster_raw")
    ...
    assign("womens_college_basketball_roster", roster_df, envir = .GlobalEnv)

    message(sprintf("womens_college_basketball roster data assigned to: womens_college_basketball_roster (Team ID: %s, %d players)",
                    team_id, nrow(roster_df)))

    return(invisible(roster_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch womens_college_basketball roster data for team %s: %s", team_id, e$message))
  })
}
