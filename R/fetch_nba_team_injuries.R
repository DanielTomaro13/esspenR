#' Fetch NBA team injuries data using Core API
#'
#' Retrieves current injury information for a specific NBA team from ESPN's Core API.
#' The function fetches detailed injury reports including player status, injury type,
#' and recovery timeline information.
#'
#' @param team_id Character or Numeric. ESPN NBA team ID (required).
#'   Examples: "1" for Atlanta Hawks, "2" for Boston Celtics, "5" for Chicago Bulls.
#'   See ESPN's NBA team directory for complete team ID list.
#' @param raw Logical. If TRUE, assigns raw JSON response to global environment
#'   as 'nba_injuries_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns the processed injury data frame. The main purpose is global
#'   environment assignment of:
#'   \itemize{
#'     \item \code{nba_team_injuries} - Clean injury data frame with player details
#'     \item \code{nba_injuries_raw} - Raw JSON response (if raw = TRUE)
#'   }
#'
#' @details
...
#' fetch_nba_team_injuries(team_id = "2")
#' head(nba_team_injuries)
#'
#' # Get Los Angeles Lakers injuries
#' fetch_nba_team_injuries(team_id = "13")
#'
#' # View injury types and statuses
#' table(nba_team_injuries$injury_status)
#' table(nba_team_injuries$injury_type_name)
#'
#' # Get raw data for custom processing
#' fetch_nba_team_injuries(team_id = "2", raw = TRUE)
#' str(nba_injuries_raw, max.level = 2)
#'
#' # Analyze current active injuries
#' active_injuries <- nba_team_injuries[
#'   nba_team_injuries$injury_status %in% c("Out", "Questionable", "Doubtful"),
...
#' @export
fetch_nba_team_injuries <- function(team_id, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL for NBA Core API
  url <- sprintf("https://sports.core.api.espn.com/v2/sports/basketball/leagues/nba/teams/%s/injuries", team_id)

  message(sprintf("Fetching NBA injuries for team ID: %s...", team_id))
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("nba_injuries_raw", data, envir = .GlobalEnv)
      message("Raw NBA injuries data assigned to: nba_injuries_raw")
    ...
    injuries_df <- create_clean_nba_team_injuries_dataset(data, team_id)

    # Assign to global environment
    assign("nba_team_injuries", injuries_df, envir = .GlobalEnv)
    message(sprintf("NBA team injuries assigned to: nba_team_injuries (Team ID: %s, %d injuries)",
    ...
    stop(sprintf("Failed to fetch NBA injuries for team %s: %s", team_id, e$message))
  })
}

#' Create clean NBA team injuries dataset from ESPN Core API response
#'
...
#' @keywords internal
create_clean_nba_team_injuries_dataset <- function(data, team_id) {
...
#' Fetch multiple NBA team injuries data
#'
#' Retrieves injury information for multiple NBA teams with rate limiting to
#' be respectful to ESPN's Core API. This function calls \code{\link{fetch_nba_team_injuries}}
#' for each team and combines the results.
#'
#' @param team_ids Character or Numeric vector. ESPN NBA team IDs.
...
#' fetch_multiple_nba_team_injuries(east_teams)
#'
#' # Use longer delay for larger requests
#' all_teams <- c("1", "2", "3", "4", "5")
#' fetch_multiple_nba_team_injuries(all_teams, delay = 0.5)
#'
#' # Analyze combined results
#' unique_teams <- unique(nba_team_injuries$request_team_id)
#' cat("Retrieved injury data for", length(unique_teams), "teams\n")
#'
#' # League-wide injury analysis
#' injury_summary <- table(nba_team_injuries$injury_type_name)
#' print(injury_summary)
#'
#' # Teams with most injuries
#' team_injury_counts <- table(nba_team_injuries$request_team_id)
#' print(sort(team_injury_counts, decreasing = TRUE))
#' }
#'
#' @importFrom httr GET status_code content timeout
#' @export
fetch_multiple_nba_team_injuries <- function(team_ids, delay = 0.3, raw = FALSE) {
  # Input validation
  ...
  message(sprintf("Starting to fetch NBA injury data for %d teams...", length(team_ids)))

  # Process each team sequentially
  for (i in seq_along(team_ids)) {
    team_id <- team_ids[i]
    message(sprintf("Fetching NBA injuries for team %s (%d/%d)...", team_id, i, length(team_ids)))

    tryCatch({
      # Fetch individual team data
      team_data <- fetch_nba_team_injuries(
        team_id = team_id,
        raw = raw
      )
      ...
      # Combine data
      injuries_df <- get("nba_team_injuries", envir = .GlobalEnv)
      all_injuries <- rbind(all_injuries, injuries_df)

    }, error = function(e) {
      message(sprintf("Failed to fetch NBA injuries for team %s: %s", team_id, e$message))
    })
  ...
  if (nrow(all_injuries) > 0) {
    ...
    assign("nba_team_injuries", all_injuries, envir = .GlobalEnv)

    unique_teams <- length(unique(all_injuries$request_team_id))
    total_injuries <- nrow(all_injuries)

    message(sprintf("Combined NBA team injuries assigned to: nba_team_injuries (%d teams, %d injuries)",
