#' Fetch womens_college_basketball team injuries data using Core API
#'
#' Retrieves current injury information for a specific womens_college_basketball team from ESPN's Core API.
#' The function fetches detailed injury reports including player status, injury type,
#' and recovery timeline information.
#'
#' @param team_id Character or Numeric. ESPN womens_college_basketball team ID (required).
#'   Examples: "1" for Atlanta Hawks, "2" for Boston Celtics, "5" for Chicago Bulls.
#'   See ESPN's womens_college_basketball team directory for complete team ID list.
#' @param raw Logical. If TRUE, assigns raw JSON response to global environment
#'   as 'womens_college_basketball_injuries_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns the processed injury data frame. The main purpose is global
#'   environment assignment of:
#'   \itemize{
#'     \item \code{womens_college_basketball_team_injuries} - Clean injury data frame with player details
#'     \item \code{womens_college_basketball_injuries_raw} - Raw JSON response (if raw = TRUE)
#'   }
#'
#' @details
...
#' fetch_womens_college_basketball_team_injuries(team_id = "2")
#' head(womens_college_basketball_team_injuries)
#'
#' # Get Los Angeles Lakers injuries
#' fetch_womens_college_basketball_team_injuries(team_id = "13")
#'
#' # View injury types and statuses
#' table(womens_college_basketball_team_injuries$injury_status)
#' table(womens_college_basketball_team_injuries$injury_type_name)
#'
#' # Get raw data for custom processing
#' fetch_womens_college_basketball_team_injuries(team_id = "2", raw = TRUE)
#' str(womens_college_basketball_injuries_raw, max.level = 2)
#'
#' # Analyze current active injuries
#' active_injuries <- womens_college_basketball_team_injuries[
#'   womens_college_basketball_team_injuries$injury_status %in% c("Out", "Questionable", "Doubtful"),
...
#' @export
fetch_womens_college_basketball_team_injuries <- function(team_id, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL for womens_college_basketball Core API
  url <- sprintf("https://sports.core.api.espn.com/v2/sports/basketball/leagues/womens-college-basketball/teams/%s/injuries", team_id)

  message(sprintf("Fetching womens_college_basketball injuries for team ID: %s...", team_id))
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("womens_college_basketball_injuries_raw", data, envir = .GlobalEnv)
      message("Raw womens_college_basketball injuries data assigned to: womens_college_basketball_injuries_raw")
    ...
    injuries_df <- create_clean_womens_college_basketball_team_injuries_dataset(data, team_id)

    # Assign to global environment
    assign("womens_college_basketball_team_injuries", injuries_df, envir = .GlobalEnv)
    message(sprintf("womens_college_basketball team injuries assigned to: womens_college_basketball_team_injuries (Team ID: %s, %d injuries)",
    ...
    stop(sprintf("Failed to fetch womens_college_basketball injuries for team %s: %s", team_id, e$message))
  })
}

#' Create clean womens_college_basketball team injuries dataset from ESPN Core API response
#'
...
#' @keywords internal
create_clean_womens_college_basketball_team_injuries_dataset <- function(data, team_id) {
...
#' Fetch multiple womens_college_basketball team injuries data
#'
#' Retrieves injury information for multiple womens_college_basketball teams with rate limiting to
#' be respectful to ESPN's Core API. This function calls \code{\link{fetch_womens_college_basketball_team_injuries}}
#' for each team and combines the results.
#'
#' @param team_ids Character or Numeric vector. ESPN womens_college_basketball team IDs.
...
#' fantasy basketball research, or medical staff benchmarking studies.
#'
#' @examples
#' \dontrun{
#' # Get injuries for multiple teams
#' east_teams <- c("1", "2", "17")  # Hawks, Celtics, Nets
#' fetch_multiple_womens_college_basketball_team_injuries(east_teams)
#'
#' # Use longer delay for larger requests
#' all_teams <- c("1", "2", "3", "4", "5")
#' fetch_multiple_womens_college_basketball_team_injuries(all_teams, delay = 0.5)
#'
#' # Analyze combined results
#' unique_teams <- unique(womens_college_basketball_team_injuries$request_team_id)
#' cat("Retrieved injury data for", length(unique_teams), "teams\n")
#'
#' # League-wide injury analysis
#' injury_summary <- table(womens_college_basketball_team_injuries$injury_type_name)
#' print(injury_summary)
#'
#' # Teams with most injuries
#' team_injury_counts <- table(womens_college_basketball_team_injuries$request_team_id)
#' print(sort(team_injury_counts, decreasing = TRUE))
#' }
#'
#' @importFrom httr GET status_code content timeout
#' @export
fetch_multiple_womens_college_basketball_team_injuries <- function(team_ids, delay = 0.3, raw = FALSE) {
  # Input validation
  ...
  message(sprintf("Starting to fetch womens_college_basketball injury data for %d teams...", length(team_ids)))

  # Process each team sequentially
  for (i in seq_along(team_ids)) {
    team_id <- team_ids[i]
    message(sprintf("Fetching womens_college_basketball injuries for team %s (%d/%d)...", team_id, i, length(team_ids)))

    tryCatch({
      # Fetch individual team data
      team_data <- fetch_womens_college_basketball_team_injuries(
        team_id = team_id,
        raw = raw
      )
      ...
      # Combine data
      injuries_df <- get("womens_college_basketball_team_injuries", envir = .GlobalEnv)
      all_injuries <- rbind(all_injuries, injuries_df)

    }, error = function(e) {
      message(sprintf("Failed to fetch womens_college_basketball injuries for team %s: %s", team_id, e$message))
    })
  ...
  if (nrow(all_injuries) > 0) {
    ...
    assign("womens_college_basketball_team_injuries", all_injuries, envir = .GlobalEnv)

    unique_teams <- length(unique(all_injuries$request_team_id))
    total_injuries <- nrow(all_injuries)

    message(sprintf("Combined womens_college_basketball team injuries assigned to: womens_college_basketball_team_injuries (%d teams, %d injuries)",
