#' Fetch NFL scoreboard data using Site API
#'
...
#'
#' @examples
#' # Get NFL scoreboard for specific week
#' fetch_nfl_scoreboard(year = 2025, week = 1)
#' head(nfl_scoreboard)
#'
#' # Get current week scoreboard
#' fetch_nfl_scoreboard(year = 2025)
#'
#' # Get raw data
#' fetch_nfl_scoreboard(year = 2025, week = 1, raw = TRUE)
#'
fetch_nfl_scoreboard <- function(year, week = NULL, seasontype = 2, raw = FALSE) {
  # Validate inputs
  if (missing(year)) {
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("nfl_scoreboard_raw", data, envir = .GlobalEnv)
      message("Raw NFL scoreboard data assigned to: nfl_scoreboard_raw")
      return(invisible(data))
    }

    # Create clean scoreboard dataset
    scoreboard_df <- create_clean_scoreboard_dataset(data, year, week, seasontype)

    # Assign to global environment
    assign("nfl_scoreboard", scoreboard_df, envir = .GlobalEnv)

    message(sprintf("NFL scoreboard data assigned to: nfl_scoreboard (%d games)", nrow(scoreboard_df)))

    return(invisible(scoreboard_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch NFL scoreboard data: %s", e$message))
  })
}

#' Create clean scoreboard dataset from ESPN Site API response
#'
...
create_clean_scoreboard_dataset <- function(data, year, week, seasontype) {
