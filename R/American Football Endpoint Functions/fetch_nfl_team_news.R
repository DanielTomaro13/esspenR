#' Safe nested data extraction helper function for NFL team news
#'
...
#' @keywords internal
extract_nested_news <- function(data, path, default = NA) {
  tryCatch({
    result <- data
    for (key in path) {
...
#' Create NFL team news data frame from Site API response
#'
...
#' @keywords internal
create_nfl_news_dataset <- function(data, team_id) {
  ...
  # Extract articles
  articles <- extract_nested_news(data, c("articles"), list())

  for (i in seq_along(articles)) {
    article <- articles[[i]]

    # Basic article information
    article_id <- extract_nested_news(article, c("id"), NA_character_)
    headline <- extract_nested_news(article, c("headline"), NA_character_)
    description <- extract_nested_news(article, c("description"), NA_character_)
    story <- extract_nested_news(article, c("story"), NA_character_)
    published <- extract_nested_news(article, c("published"), NA_character_)
    last_modified <- extract_nested_news(article, c("lastModified"), NA_character_)
    premium <- extract_nested_news(article, c("premium"), "false")
    byline <- extract_nested_news(article, c("byline"), NA_character_)
    data_source_identifier <- extract_nested_news(article, c("dataSourceIdentifier"), NA_character_)
    section <- extract_nested_news(article, c("section"), NA_character_)
    type <- extract_nested_news(article, c("type"), NA_character_)

    # Keywords
    keywords_list <- extract_nested_news(article, c("keywords"), list())
    ...
    links <- extract_nested_news(article, c("links"), list())
    url <- NA_character_

    if (length(links) > 0) {
      web_link <- links[["web"]]
      if (!is.null(web_link)) {
        url <- extract_nested_news(web_link, c("href"), NA_character_)
      }
    }
    ...
    video <- extract_nested_news(article, c("video"), list())
    if (length(video) > 0 && !is.null(video)) {
      video_id <- extract_nested_news(video, c("id"), NA_character_)
      video_headline <- extract_nested_news(video, c("headline"), NA_character_)
      video_caption <- extract_nested_news(video, c("caption"), NA_character_)
      video_description <- extract_nested_news(video, c("description"), NA_character_)
      video_premium <- extract_nested_news(video, c("premium"), "false")
      video_duration <- extract_nested_news(video, c("duration"), NA_character_)
    }
    ...
    images <- extract_nested_news(article, c("images"), list())
    if (length(images) > 0) {
      # Take the first image
      first_image <- images[[1]]
      if (!is.null(first_image)) {
        image_id <- extract_nested_news(first_image, c("id"), NA_character_)
        image_name <- extract_nested_news(first_image, c("name"), NA_character_)
        image_caption <- extract_nested_news(first_image, c("caption"), NA_character_)
        image_url <- extract_nested_news(first_image, c("url"), NA_character_)
        image_width <- extract_nested_news(first_image, c("width"), NA_character_)
        image_height <- extract_nested_news(first_image, c("height"), NA_character_)
      }
    }

    # Clean text content
    headline <- clean_text_content(headline)
    description <- clean_text_content(description)
    story <- clean_text_content(story)
    byline <- clean_text_content(byline)
    video_headline <- clean_text_content(video_headline)
    video_caption <- clean_text_content(video_caption)
    video_description <- clean_text_content(video_description)
    image_caption <- clean_text_content(image_caption)
...
#' Get NFL team ID mapping
#'
#' Returns a mapping of team abbreviations to team IDs for ESPN API
#' @return Named vector with team abbreviations as names and IDs as values
#' @export
get_nfl_team_ids <- function() {
  team_ids <- c(
...
#' Fetch NFL team-specific news using Site API
#'
#' Retrieves team-specific news articles from ESPN's Site API.
#' The function fetches comprehensive news information including articles,
#' videos, images, and metadata for a specific NFL team.
#'
#' @param team Character. Team abbreviation (e.g., "KC", "SF") or team ID.
...
#'   as 'nfl_team_news_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns the processed data frame. The main purpose is global
#'   environment assignment of \code{nfl_team_news} containing:
#'   \itemize{
...
#'
#' **Team ID Mapping**:
#' The function accepts both team abbreviations (e.g., "KC", "SF") and team IDs.
#' Use \code{get_nfl_team_ids()} to see all available team mappings.
#'
#' @examples
#' \dontrun{
#' # Get news for Kansas City Chiefs
#' fetch_nfl_team_news("KC")
#'
#' # Get news using team ID
#' fetch_nfl_team_news("12")
#'
#' # Get limited number of articles
#' fetch_nfl_team_news("SF", limit = 20)
#'
#' # Check the data
#' head(nfl_team_news)
#'
#' # View recent headlines
#' recent_news <- nfl_team_news[1:10, c("headline", "published", "byline")]
#' print(recent_news)
#'
#' # Find video content
#' video_news <- nfl_team_news[!is.na(nfl_team_news$video_id),
#'                            c("headline", "video_headline", "video_duration")]
#' print(video_news)
#'
#' # Get articles with images
#' image_news <- nfl_team_news[!is.na(nfl_team_news$image_url),
...
#' }
#'
#' @seealso \code{\link{get_nfl_team_ids}} for team ID mappings
#'
#' @export
fetch_nfl_team_news <- function(team, limit = 50, raw = FALSE) {

  # Input validation
  if (missing(team) || is.null(team) || team == "") {
    stop("'team' parameter is required. Provide team abbreviation (e.g., 'KC') or team ID.")
  }
  ...
  # Build API URL
  url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/football/nfl/news?team=%s&limit=%d",
                 team_id, limit)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("nfl_team_news_raw", data, envir = .GlobalEnv)
      message("Raw NFL team news data assigned to: nfl_team_news_raw")
    ...
    # Create news dataset
    news_df <- create_nfl_news_dataset(data, team_id)

    # Assign to global environment
    assign("nfl_team_news", news_df, envir = .GlobalEnv)
    ...
    message(sprintf("NFL team news assigned to: nfl_team_news (%d articles)", total_articles))
    ...
    stop(sprintf("Failed to fetch NFL team news for team %s: %s", team_abbr, e$message))
  })
}

#' Fetch NFL news for multiple teams
#'
#' Retrieves team-specific news for multiple NFL teams with rate limiting.
#' This function calls \code{\link{fetch_nfl_team_news}} for each team
...
#'   environment assignment of combined \code{nfl_team_news} from all teams.
#'
#' @examples
#' \dontrun{
#' # Get news for division rivals
#' fetch_multiple_nfl_team_news(c("KC", "LV", "LAC", "DEN"))
#'
#' # Get news for specific teams with fewer articles each
#' fetch_multiple_nfl_team_news(c("SF", "SEA", "LAR"), limit = 15)
#'
#' # Check combined results
#' head(nfl_team_news)
#' table(nfl_team_news$team_id)
#' }
#'
#' @seealso \code{\link{fetch_nfl_team_news}} for single team data
#' @export
fetch_multiple_nfl_team_news <- function(teams, limit = 25, delay = 0.5, raw = FALSE) {
  # Input validation
  if (length(teams) == 0) {
  ...
  message(sprintf("Starting to fetch NFL team news for %d teams...", length(teams)))

  # Process each team sequentially
  for (i in seq_along(teams)) {
    team <- teams[i]
    message(sprintf("Fetching NFL team news for %s (%d/%d)...", team, i, length(teams)))

    tryCatch({
      # Fetch individual team data
      team_data <- fetch_nfl_team_news(
        team = team,
        limit = limit,
        raw = raw
      )
      ...
      # Combine data
      news_df <- get("nfl_team_news", envir = .GlobalEnv)
      all_news <- rbind(all_news, news_df)

    }, error = function(e) {
      message(sprintf("Failed to fetch NFL team news for %s: %s", team, e$message))
    })
  ...
  if (nrow(all_news) > 0) {
    ...
    assign("nfl_team_news", all_news, envir = .GlobalEnv)

    unique_teams <- length(unique(all_news$team_id))
    total_articles <- nrow(all_news)

    message(sprintf("Combined NFL team news assigned to: nfl_team_news (%d teams, %d articles)",
