#' Safe nested data extraction helper function for college_baseball teams
#'
...
#' @keywords internal
extract_nested_college_baseball_teams <- function(data, path, default = NA) {
  tryCatch({
    result <- data
    for (key in path) {
...
#' Create college_baseball teams data frame from Site API response
#'
...
#' @keywords internal
create_college_baseball_teams_dataset <- function(data) {
  ...
  for (i in seq_along(teams_data)) {
    ...
    team_id <- extract_nested_college_baseball_teams(team, c("id"), NA_character_)
    team_uid <- extract_nested_college_baseball_teams(team, c("uid"), NA_character_)
    team_slug <- extract_nested_college_baseball_teams(team, c("slug"), NA_character_)
    team_abbreviation <- extract_nested_college_baseball_teams(team, c("abbreviation"), NA_character_)
    team_display_name <- extract_nested_college_baseball_teams(team, c("displayName"), NA_character_)
    team_short_display_name <- extract_nested_college_baseball_teams(team, c("shortDisplayName"), NA_character_)
    team_name <- extract_nested_college_baseball_teams(team, c("name"), NA_character_)
    team_nickname <- extract_nested_college_baseball_teams(team, c("nickname"), NA_character_)
    team_location <- extract_nested_college_baseball_teams(team, c("location"), NA_character_)
    team_color <- extract_nested_college_baseball_teams(team, c("color"), NA_character_)
    team_alternate_color <- extract_nested_college_baseball_teams(team, c("alternateColor"), NA_character_)
    team_is_active <- extract_nested_college_baseball_teams(team, c("isActive"), "true")
    team_is_all_star <- extract_nested_college_baseball_teams(team, c("isAllStar"), "false")
    ...
    logos <- extract_nested_college_baseball_teams(team, c("logos"), list())
    if (length(logos) > 0) {
      for (logo in logos) {
        logo_rel <- extract_nested_college_baseball_teams(logo, c("rel"), list())
        if (length(logo_rel) > 0) {
          if ("default" %in% logo_rel || "full" %in% logo_rel) {
            team_logo_href <- extract_nested_college_baseball_teams(logo, c("href"), team_logo_href)
            team_logo_alt <- extract_nested_college_baseball_teams(logo, c("alt"), team_logo_alt)
            team_logo_width <- extract_nested_college_baseball_teams(logo, c("width"), team_logo_width)
            team_logo_height <- extract_nested_college_baseball_teams(logo, c("height"), team_logo_height)
          } else if ("dark" %in% logo_rel) {
            team_logo_dark_href <- extract_nested_college_baseball_teams(logo, c("href"), team_logo_dark_href)
          }
        }
      }
    }
    ...
    conference_info <- extract_nested_college_baseball_teams(team, c("conference"), list())
    if (length(conference_info) > 0) {
      conference_id <- extract_nested_college_baseball_teams(conference_info, c("id"), NA_character_)
      conference_name <- extract_nested_college_baseball_teams(conference_info, c("name"), NA_character_)
      conference_short_name <- extract_nested_college_baseball_teams(conference_info, c("shortName"), NA_character_)
      conference_uid <- extract_nested_college_baseball_teams(conference_info, c("uid"), NA_character_)
      conference_abbreviation <- extract_nested_college_baseball_teams(conference_info, c("abbreviation"), NA_character_)
    } else {
      # Try groups array for conference information
      groups <- extract_nested_college_baseball_teams(team, c("groups"), list())
      if (length(groups) > 0) {
        # Find conference group
        for (group in groups) {
          is_conf_group <- extract_nested_college_baseball_teams(group, c("isConferenceGroup"), "false")
          if (is_conf_group == "true") {
            conference_id <- extract_nested_college_baseball_teams(group, c("id"), NA_character_)
            conference_name <- extract_nested_college_baseball_teams(group, c("name"), NA_character_)
            conference_short_name <- extract_nested_college_baseball_teams(group, c("shortName"), NA_character_)
            conference_uid <- extract_nested_college_baseball_teams(group, c("uid"), NA_character_)
            conference_abbreviation <- extract_nested_college_baseball_teams(group, c("abbreviation"), NA_character_)
            break
          }
        }
      }
    ...
    division_info <- extract_nested_college_baseball_teams(team, c("division"), list())
    if (length(division_info) > 0) {
      division_id <- extract_nested_college_baseball_teams(division_info, c("id"), NA_character_)
      division_name <- extract_nested_college_baseball_teams(division_info, c("name"), NA_character_)
      division_short_name <- extract_nested_college_baseball_teams(division_info, c("shortName"), NA_character_)
      division_uid <- extract_nested_college_baseball_teams(division_info, c("uid"), NA_character_)
      division_abbreviation <- extract_nested_college_baseball_teams(division_info, c("abbreviation"), NA_character_)
    } else {
      ...
      if (length(groups) > 0) {
        ...
        for (group in groups) {
          ...
          if (is_conf_group == "false") {
            division_id <- extract_nested_college_baseball_teams(group, c("id"), NA_character_)
            division_name <- extract_nested_college_baseball_teams(group, c("name"), NA_character_)
            division_short_name <- extract_nested_college_baseball_teams(group, c("shortName"), NA_character_)
            division_uid <- extract_nested_college_baseball_teams(group, c("uid"), NA_character_)
            division_abbreviation <- extract_nested_college_baseball_teams(group, c("abbreviation"), NA_character_)
            break
          }
        }
      }
    ...
    venue_info <- extract_nested_college_baseball_teams(team, c("venue"), list())
    if (length(venue_info) > 0) {
      venue_id <- extract_nested_college_baseball_teams(venue_info, c("id"), NA_character_)
      venue_name <- extract_nested_college_baseball_teams(venue_info, c("shortName"), NA_character_)
      venue_full_name <- extract_nested_college_baseball_teams(venue_info, c("fullName"), NA_character_)
      if (is.na(venue_name)) {
        venue_name <- extract_nested_college_baseball_teams(venue_info, c("name"), NA_character_)
      }
      venue_capacity <- extract_nested_college_baseball_teams(venue_info, c("capacity"), NA_character_)
      venue_indoor <- extract_nested_college_baseball_teams(venue_info, c("indoor"), "true")  # college_baseball is typically indoor

      # Address information
      address_info <- extract_nested_college_baseball_teams(venue_info, c("address"), list())
      if (length(address_info) > 0) {
        venue_city <- extract_nested_college_baseball_teams(address_info, c("city"), NA_character_)
        venue_state <- extract_nested_college_baseball_teams(address_info, c("state"), NA_character_)
        venue_zip_code <- extract_nested_college_baseball_teams(address_info, c("zipCode"), NA_character_)
        venue_country_code <- extract_nested_college_baseball_teams(address_info, c("countryCode"), NA_character_)
      }
    }
...
#' Fetch college_baseball teams using Site API
#'
#' Retrieves comprehensive college_baseball teams information from ESPN's Site API.
...
#'   as 'college_baseball_teams_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns the processed data frame. The main purpose is global
#'   environment assignment of \code{college_baseball_teams} containing:
#'   \itemize{
...
#'
#' @details
#' The function creates a comprehensive data frame with all college_baseball teams information.
...
#'   \item Features: indoor status (typically true for college_baseball)
#' }
#'
#' @examples
#' \dontrun{
#' # Get all college_baseball teams
#' fetch_college_baseball_teams()
#'
#' # Check the data
#' head(college_baseball_teams)
#' str(college_baseball_teams)
#'
#' # View team identity and branding
#' team_identity <- college_baseball_teams[, c("team_abbreviation", "team_display_name",
#'                               "team_location", "team_color", "team_alternate_color")]
#' print("Team identity:")
#' print(team_identity)
#'
#' # View conference and division breakdown
#' conf_division <- college_baseball_teams[, c("team_abbreviation", "team_display_name",
...
#'   conference_breakdown <- college_baseball_teams %>%
...
#'
#'   # Division breakdown
#'   division_breakdown <- college_baseball_teams %>%
...
#'
#' # Venue analysis
#' venue_info <- college_baseball_teams[!is.na(college_baseball_teams$venue_full_name),
...
#' eastern_teams <- college_baseball_teams[college_baseball_teams$conference_name == "Eastern Conference" &
#'                           !is.na(college_baseball_teams$conference_name), ]
#'
...
#' western_teams <- college_baseball_teams[college_baseball_teams$conference_name == "Western Conference" &
#'                           !is.na(college_baseball_teams$conference_name), ]
#'
...
#' colors_data <- college_baseball_teams[!is.na(college_baseball_teams$team_color), ]
...
#' large_venues <- college_baseball_teams[!is.na(college_baseball_teams$venue_capacity) & college_baseball_teams$venue_capacity != "", ]
...
#' lakers_info <- college_baseball_teams[college_baseball_teams$team_abbreviation == "LAL", ]
...
#'
#' @export
fetch_college_baseball_teams <- function(raw = FALSE) {
  ...
  tryCatch({
    message("Fetching all college_baseball teams...")
    ...
    if (isTRUE(raw)) {
      assign("college_baseball_teams_raw", data, envir = .GlobalEnv)
      message("Raw college_baseball teams data assigned to: college_baseball_teams_raw")
      ...
      teams_data <- list()
      if ("sports" %in% names(data)) {
        teams_data <- extract_nested_college_baseball_teams(data, c("sports", 1, "leagues", 1, "teams"), list())
        if (length(teams_data) > 0) {
      ...
      if (length(teams_data) > 0) {
        ...
        if (length(teams_data) > 0) {
          ...
          team_name <- extract_nested_college_baseball_teams(first_team, c("displayName"), "Unknown")
          team_conf <- extract_nested_college_baseball_teams(first_team, c("conference", "name"), "Unknown")
          venue_name <- extract_nested_college_baseball_teams(first_team, c("venue", "fullName"), "Unknown")
    ...
    teams_df <- create_college_baseball_teams_dataset(data)

    # Assign to global environment
    assign("college_baseball_teams", teams_df, envir = .GlobalEnv)

    # Summary message
    total_teams <- nrow(teams_df)

    message(sprintf("college_baseball teams assigned to: college_baseball_teams (%d teams)", total_teams))
    ...
    stop(sprintf("Failed to fetch college_baseball teams: %s", e$message))
  })
}
