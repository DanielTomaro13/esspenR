#' Safe nested data extraction helper function for college_baseball boxscore data
#'
...
#' @keywords internal
extract_nested_college_baseball_boxscore <- function(data, path, default = NA) {
  tryCatch({
    result <- data
    for (key in path) {
...
#' Create college_baseball player statistics data frame from ESPN boxscore API
#'
...
#' @keywords internal
create_college_baseball_player_stats_dataset <- function(data, event_id) {
  ...
  boxscore <- extract_nested_college_baseball_boxscore(data, c("boxscore"), list())
  ...
  players_data <- extract_nested_college_baseball_boxscore(boxscore, c("players"), list())
...
#' Create college_baseball team statistics data frame from ESPN boxscore API
#'
...
#' @keywords internal
create_college_baseball_team_stats_dataset <- function(data, event_id) {
  ...
  teams_data <- extract_nested_college_baseball_boxscore(boxscore, c("teams"), list())
  ...
  for (i in seq_len(nrow(teams_data))) {
    ...
    header <- extract_nested_college_baseball_boxscore(data, c("header"), list())
    if (length(header) > 0) {
      competitions <- extract_nested_college_baseball_boxscore(header, c("competitions"), list())
      if (length(competitions) > 0) {
        competitors <- extract_nested_college_baseball_boxscore(competitions[[1]], c("competitors"), list())
        if (length(competitors) > 0) {
          for (comp in competitors) {
            comp_team <- extract_nested_college_baseball_boxscore(comp, c("team"), list())
            if (extract_nested_college_baseball_boxscore(comp_team, c("id"), "") == team_id) {
              team_score <- extract_nested_college_baseball_boxscore(comp, c("score"), NA_character_)
              break
            }
          }
        }
...
#' Fetch detailed college_baseball boxscore using ESPN API
#'
#' Retrieves comprehensive boxscore information for a specific college_baseball game from ESPN's API.
...
#'   (e.g., "401581132" for a specific college_baseball game).
#' @param raw Logical. If TRUE, assigns raw JSON response to global environment
#'   as 'college_baseball_boxscore_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns a list containing the datasets. The main purpose is global
#'   environment assignment of:
#'   \itemize{
#'     \item \code{college_baseball_player_stats}: Individual player statistics for both teams
#'     \item \code{college_baseball_team_stats}: Team-level statistics and totals
#'   }
#'
#' @details
#' The function creates comprehensive datasets from ESPN's college_baseball boxscore API.
#'
#' **Player Statistics** (\code{college_baseball_player_stats}):
#' \itemize{
...
#' }
#'
#' **Team Statistics** (\code{college_baseball_team_stats}):
#' \itemize{
...
#' fetch_college_baseball_boxscore("401581132")
#'
#' # Access player statistics
#' head(college_baseball_player_stats)
#'
#' # View batting statistics only
#' batting_stats <- college_baseball_player_stats[college_baseball_player_stats$stat_type == "batting", ]
#' head(batting_stats[, c("player_display_name", "at_bats", "hits", "rbi", "avg")])
#'
#' # View pitching statistics only
#' pitching_stats <- college_baseball_player_stats[college_baseball_player_stats$stat_type == "pitching", ]
#' head(pitching_stats[, c("player_display_name", "innings_pitched", "era", "whip")])
#'
#' # Access team statistics
#' head(college_baseball_team_stats)
#'
#' # Find top hitters
#' top_hitters <- college_baseball_player_stats[
#'   college_baseball_player_stats$stat_type == "batting" &
#'   !is.na(college_baseball_player_stats$hits) &
#'   college_baseball_player_stats$hits != "",
#' ]
...
#' @export
fetch_college_baseball_boxscore <- function(event_id, raw = FALSE) {
  # Input validation
  ...
  # Construct boxscore API URL for college_baseball
  url <- paste0("https://site.api.espn.com/apis/site/v2/sports/baseball/college-baseball/summary?event=", event_id)

  message("Fetching college_baseball boxscore for game ", event_id, "...")
  ...
  if (isTRUE(raw)) {
    assign("college_baseball_boxscore_raw", data, envir = .GlobalEnv)
    message("Raw college_baseball boxscore data assigned to: college_baseball_boxscore_raw")
  ...
  player_stats <- tryCatch({
    create_college_baseball_player_stats_dataset(data, event_id)
  }, error = function(e) {
  ...
  assign("college_baseball_player_stats", player_stats, envir = .GlobalEnv)

  # Create team statistics dataset
  message("Processing team statistics...")
  team_stats <- tryCatch({
    create_college_baseball_team_stats_dataset(data, event_id)
  }, error = function(e) {
  ...
  # Assign to global environment
  assign("college_baseball_team_stats", team_stats, envir = .GlobalEnv)
