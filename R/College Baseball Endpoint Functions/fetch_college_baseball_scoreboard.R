#' Fetch college_baseball scoreboard data using Site API
#'
...
#'
#' @examples
#' # Get college_baseball scoreboard for current date
#' fetch_college_baseball_scoreboard()
#' head(college_baseball_scoreboard)
#'
#' # Get scoreboard for specific date
#' fetch_college_baseball_scoreboard(dates = "20241225")
#'
#' # Get raw data
#' fetch_college_baseball_scoreboard(raw = TRUE)
#'
fetch_college_baseball_scoreboard <- function(year = NULL, week = NULL, seasontype = 2, dates = NULL, raw = FALSE) {
  ...
  # Fetch and parse
  tryCatch({
    message("Fetching college_baseball scoreboard data...")
    ...
    if (isTRUE(raw)) {
      assign("college_baseball_scoreboard_raw", data, envir = .GlobalEnv)
      message("Raw college_baseball scoreboard data assigned to: college_baseball_scoreboard_raw")
      return(invisible(data))
    }

    # Create clean scoreboard dataset
    scoreboard_df <- create_clean_college_baseball_scoreboard_dataset(data, year, week, seasontype, dates)

    # Assign to global environment
    assign("college_baseball_scoreboard", scoreboard_df, envir = .GlobalEnv)
    message(sprintf("college_baseball scoreboard data assigned to: college_baseball_scoreboard (%d games)", nrow(scoreboard_df)))

    return(invisible(scoreboard_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch college_baseball scoreboard data: %s", e$message))
  })
}

#' Create clean college_baseball scoreboard dataset from ESPN Site API response
#'
...
create_clean_college_baseball_scoreboard_dataset <- function(data, year, week, seasontype, dates) {
