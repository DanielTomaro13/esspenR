#' Safe nested data extraction helper function for detailed college_baseball team data
#'
...
#' @keywords internal
extract_nested_college_baseball_detailed <- function(data, path, default = NA) {
  tryCatch({
    result <- data
    for (key in path) {
...
#' Create detailed college_baseball team data frame from Site API response
#'
#' Processes raw JSON response from ESPN Site API into structured data frame
#' containing comprehensive college_baseball team information
#'
...
#' @keywords internal
create_college_baseball_detailed_team_dataset <- function(data, team_identifier) {
  ...
  team_id <- extract_nested_college_baseball_detailed(team_data, c("id"), NA_character_)
  team_uid <- extract_nested_college_baseball_detailed(team_data, c("uid"), NA_character_)
  team_slug <- extract_nested_college_baseball_detailed(team_data, c("slug"), NA_character_)
  team_abbreviation <- extract_nested_college_baseball_detailed(team_data, c("abbreviation"), NA_character_)
  team_display_name <- extract_nested_college_baseball_detailed(team_data, c("displayName"), NA_character_)
  team_short_display_name <- extract_nested_college_baseball_detailed(team_data, c("shortDisplayName"), NA_character_)
  team_name <- extract_nested_college_baseball_detailed(team_data, c("name"), NA_character_)
  team_nickname <- extract_nested_college_baseball_detailed(team_data, c("nickname"), NA_character_)
  team_location <- extract_nested_college_baseball_detailed(team_data, c("location"), NA_character_)
  team_color <- extract_nested_college_baseball_detailed(team_data, c("color"), NA_character_)
  team_alternate_color <- extract_nested_college_baseball_detailed(team_data, c("alternateColor"), NA_character_)
  team_is_active <- extract_nested_college_baseball_detailed(team_data, c("isActive"), "true")
  team_is_all_star <- extract_nested_college_baseball_detailed(team_data, c("isAllStar"), "false")
  ...
  logos <- extract_nested_college_baseball_detailed(team_data, c("logos"), list())
  if (length(logos) > 0) {
    for (logo in logos) {
      logo_rel <- extract_nested_college_baseball_detailed(logo, c("rel"), list())
      if (length(logo_rel) > 0) {
        if ("default" %in% logo_rel || "full" %in% logo_rel) {
          team_logo_href <- extract_nested_college_baseball_detailed(logo, c("href"), team_logo_href)
          team_logo_alt <- extract_nested_college_baseball_detailed(logo, c("alt"), team_logo_alt)
          team_logo_width <- extract_nested_college_baseball_detailed(logo, c("width"), team_logo_width)
          team_logo_height <- extract_nested_college_baseball_detailed(logo, c("height"), team_logo_height)
        } else if ("dark" %in% logo_rel) {
          team_logo_dark_href <- extract_nested_college_baseball_detailed(logo, c("href"), team_logo_dark_href)
        }
      }
    }
  }
  ...
  venue_info <- extract_nested_college_baseball_detailed(team_data, c("venue"), list())
  if (length(venue_info) > 0) {
    venue_id <- extract_nested_college_baseball_detailed(venue_info, c("id"), NA_character_)
    venue_name <- extract_nested_college_baseball_detailed(venue_info, c("shortName"), NA_character_)
    venue_full_name <- extract_nested_college_baseball_detailed(venue_info, c("fullName"), NA_character_)
    if (is.na(venue_name)) {
      venue_name <- extract_nested_college_baseball_detailed(venue_info, c("name"), NA_character_)
    }
    venue_capacity <- extract_nested_college_baseball_detailed(venue_info, c("capacity"), NA_character_)
    venue_grass <- extract_nested_college_baseball_detailed(venue_info, c("grass"), NA_character_)
    venue_indoor <- extract_nested_college_baseball_detailed(venue_info, c("indoor"), "true")
    venue_dome <- extract_nested_college_baseball_detailed(venue_info, c("dome"), "false")

    # Address information
    address_info <- extract_nested_college_baseball_detailed(venue_info, c("address"), list())
    if (length(address_info) > 0) {
      venue_city <- extract_nested_college_baseball_detailed(address_info, c("city"), NA_character_)
      venue_state <- extract_nested_college_baseball_detailed(address_info, c("state"), NA_character_)
      venue_zip_code <- extract_nested_college_baseball_detailed(address_info, c("zipCode"), NA_character_)
      venue_country_code <- extract_nested_college_baseball_detailed(address_info, c("countryCode"), NA_character_)
    }
  }
  ...
  conference_info <- extract_nested_college_baseball_detailed(team_data, c("conference"), list())
  if (length(conference_info) > 0) {
    conference_id <- extract_nested_college_baseball_detailed(conference_info, c("id"), NA_character_)
    conference_name <- extract_nested_college_baseball_detailed(conference_info, c("name"), NA_character_)
    conference_short_name <- extract_nested_college_baseball_detailed(conference_info, c("shortName"), NA_character_)
    conference_uid <- extract_nested_college_baseball_detailed(conference_info, c("uid"), NA_character_)
    conference_abbreviation <- extract_nested_college_baseball_detailed(conference_info, c("abbreviation"), NA_character_)
  } else {
    # Try groups array for conference
    groups <- extract_nested_college_baseball_detailed(team_data, c("groups"), list())
    if (length(groups) > 0) {
      for (group in groups) {
        is_conf_group <- extract_nested_college_baseball_detailed(group, c("isConferenceGroup"), "false")
        if (is_conf_group == "true") {
          conference_id <- extract_nested_college_baseball_detailed(group, c("id"), NA_character_)
          conference_name <- extract_nested_college_baseball_detailed(group, c("name"), NA_character_)
          conference_short_name <- extract_nested_college_baseball_detailed(group, c("shortName"), NA_character_)
          conference_uid <- extract_nested_college_baseball_detailed(group, c("uid"), NA_character_)
          conference_abbreviation <- extract_nested_college_baseball_detailed(group, c("abbreviation"), NA_character_)
          break
        }
      }
    }
  ...
  division_info <- extract_nested_college_baseball_detailed(team_data, c("division"), list())
  if (length(division_info) > 0) {
    division_id <- extract_nested_college_baseball_detailed(division_info, c("id"), NA_character_)
    division_name <- extract_nested_college_baseball_detailed(division_info, c("name"), NA_character_)
    division_short_name <- extract_nested_college_baseball_detailed(division_info, c("shortName"), NA_character_)
    division_uid <- extract_nested_college_baseball_detailed(division_info, c("uid"), NA_character_)
    division_abbreviation <- extract_nested_college_baseball_detailed(division_info, c("abbreviation"), NA_character_)
  } else {
    ...
    if (length(groups) > 0) {
      for (group in groups) {
        ...
        if (is_conf_group == "false") {
          division_id <- extract_nested_college_baseball_detailed(group, c("id"), NA_character_)
          division_name <- extract_nested_college_baseball_detailed(group, c("name"), NA_character_)
          division_short_name <- extract_nested_college_baseball_detailed(group, c("shortName"), NA_character_)
          division_uid <- extract_nested_college_baseball_detailed(group, c("uid"), NA_character_)
          division_abbreviation <- extract_nested_college_baseball_detailed(group, c("abbreviation"), NA_character_)
          break
        }
      }
    }
  ...
  season_info <- extract_nested_college_baseball_detailed(team_data, c("season"), list())
  if (length(season_info) > 0) {
    current_season_year <- extract_nested_college_baseball_detailed(season_info, c("year"), NA_character_)
    current_season_type <- extract_nested_college_baseball_detailed(season_info, c("type"), NA_character_)
    current_season_name <- extract_nested_college_baseball_detailed(season_info, c("name"), NA_character_)
  }
  ...
  records <- extract_nested_college_baseball_detailed(team_data, c("record"), list())
  if (length(records) == 0) {
    records <- extract_nested_college_baseball_detailed(team_data, c("records"), list())
  }
  ...
  if (length(records) > 0) {
    ...
    if (is.list(records) && !is.null(names(records)) && "wins" %in% names(records)) {
      ...
      record_overall_wins <- extract_nested_college_baseball_detailed(records, c("wins"), NA_character_)
      record_overall_losses <- extract_nested_college_baseball_detailed(records, c("losses"), NA_character_)
      record_overall_ties <- extract_nested_college_baseball_detailed(records, c("ties"), NA_character_)
      record_overall_percentage <- extract_nested_college_baseball_detailed(records, c("winPercentage"), NA_character_)
      record_overall_summary <- extract_nested_college_baseball_detailed(records, c("summary"), NA_character_)
    } else if (is.list(records)) {
      # Array of records
      for (record in records) {
        record_type <- extract_nested_college_baseball_detailed(record, c("type"), NA_character_)
        record_name <- extract_nested_college_baseball_detailed(record, c("name"), NA_character_)

        if (!is.na(record_type) || !is.na(record_name)) {
          if (record_type == "total" || record_name == "overall") {
            record_overall_wins <- extract_nested_college_baseball_detailed(record, c("wins"), NA_character_)
            record_overall_losses <- extract_nested_college_baseball_detailed(record, c("losses"), NA_character_)
            record_overall_ties <- extract_nested_college_baseball_detailed(record, c("ties"), NA_character_)
            record_overall_percentage <- extract_nested_college_baseball_detailed(record, c("winPercentage"), NA_character_)
            record_overall_summary <- extract_nested_college_baseball_detailed(record, c("summary"), NA_character_)
          } else if (record_type == "home" || record_name == "home") {
            record_home_wins <- extract_nested_college_baseball_detailed(record, c("wins"), NA_character_)
            record_home_losses <- extract_nested_college_baseball_detailed(record, c("losses"), NA_character_)
            record_home_summary <- extract_nested_college_baseball_detailed(record, c("summary"), NA_character_)
          } else if (record_type == "road" || record_name == "road" || record_name == "away") {
            record_away_wins <- extract_nested_college_baseball_detailed(record, c("wins"), NA_character_)
            record_away_losses <- extract_nested_college_baseball_detailed(record, c("losses"), NA_character_)
            record_away_summary <- extract_nested_college_baseball_detailed(record, c("summary"), NA_character_)
          } else if (record_type == "conference" || record_name == "conference") {
            record_conference_wins <- extract_nested_college_baseball_detailed(record, c("wins"), NA_character_)
            record_conference_losses <- extract_nested_college_baseball_detailed(record, c("losses"), NA_character_)
            record_conference_summary <- extract_nested_college_baseball_detailed(record, c("summary"), NA_character_)
          }
        }
      }
    }
  ...
  standings <- extract_nested_college_baseball_detailed(team_data, c("standings"), list())
  if (length(standings) > 0) {
    ...
    if ("entries" %in% names(standings)) {
      ...
      if (length(standings_entries) > 0) {
        ...
        standing_conference_rank <- extract_nested_college_baseball_detailed(entry, c("stats", "conferenceRank"), NA_character_)
        standing_division_rank <- extract_nested_college_baseball_detailed(entry, c("stats", "divisionRank"), NA_character_)
        standing_league_rank <- extract_nested_college_baseball_detailed(entry, c("stats", "leagueRank"), NA_character_)
        standing_games_played <- extract_nested_college_baseball_detailed(entry, c("stats", "gamesPlayed"), NA_character_)
        standing_games_behind <- extract_nested_college_baseball_detailed(entry, c("stats", "gamesBehind"), NA_character_)
        standing_wins <- extract_nested_college_baseball_detailed(entry, c("stats", "wins"), NA_character_)
        standing_losses <- extract_nested_college_baseball_detailed(entry, c("stats", "losses"), NA_character_)
        standing_win_percentage <- extract_nested_college_baseball_detailed(entry, c("stats", "winPercent"), NA_character_)
        standing_playoff_seed <- extract_nested_college_baseball_detailed(entry, c("stats", "playoffSeed"), NA_character_)
      }
    }
  }
...
#' Fetch detailed college_baseball team information using Site API
#'
#' Retrieves comprehensive information for a specific college_baseball team from ESPN's Site API.
...
#'   as 'college_baseball_team_detailed_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns the processed data frame. The main purpose is global
#'   environment assignment of \code{college_baseball_team_detailed} containing:
#'   \itemize{
...
#'
#' @details
#' The function creates a comprehensive data frame with detailed college_baseball team information.
...
#'   \item Structure: hierarchical organization within college_baseball
#' }
#'
#' **Venue Information**:
#' \itemize{
...
#' fetch_college_baseball_team_detailed("LAL")
#'
#' # Get team by ID
#' fetch_college_baseball_team_detailed("13")
#'
#' # Get multiple teams (call individually)
#' teams_to_fetch <- c("LAL", "BOS", "GSW", "MIA")
#' for(team in teams_to_fetch) {
#'   fetch_college_baseball_team_detailed(team)
#'   # Process data here or combine later
#' }
#'
#' # Check the detailed data
#' head(college_baseball_team_detailed)
#'
#' # View team identity and branding
#' team_identity <- college_baseball_team_detailed[, c("team_abbreviation", "team_display_name",
#'                                       "team_location", "team_color", "team_alternate_color")]
#' print("Team identity:")
#' print(team_identity)
#'
#' # View venue information
#' venue_info <- college_baseball_team_detailed[, c("venue_name", "venue_full_name", "venue_capacity",
#'                                    "venue_city", "venue_state", "venue_indoor")]
#' print("Venue information:")
#' print(venue_info)
#'
#' # View conference and division information
#' org_info <- college_baseball_team_detailed[, c("conference_name", "conference_short_name",
#'                                  "division_name", "division_short_name")]
#' print("Organizational information:")
#' print(org_info)
#'
#' # View current season records
#' record_info <- college_baseball_team_detailed[, c("current_season_year", "record_overall_summary",
...
#' standing_info <- college_baseball_team_detailed[, c("standing_conference_rank", "standing_division_rank",
...
#'
#' @export
fetch_college_baseball_team_detailed <- function(team, raw = FALSE) {
  url <- paste0("https://site.api.espn.com/apis/site/v2/sports/baseball/college-baseball/teams/", team)
  ...
  if (isTRUE(raw)) {
    assign("college_baseball_team_detailed_raw", data, envir = .GlobalEnv)
  }

  result <- create_college_baseball_detailed_team_dataset(data, team)
  assign("college_baseball_team_detailed", result, envir = .GlobalEnv)
