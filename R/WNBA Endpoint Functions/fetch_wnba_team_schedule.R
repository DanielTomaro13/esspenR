#' Fetch wnba team schedule data using Site API
#'
...
#' @param raw Logical. If TRUE, assigns raw JSON to global environment as 'wnba_schedule_raw'. Default FALSE.
...
#' fetch_wnba_team_schedule(team_id = "12")
#' head(wnba_team_schedule)
#'
#' # Get raw data for custom processing
#' fetch_wnba_team_schedule(team_id = "12", season = 2023, raw = TRUE)
#' str(wnba_schedule_raw, max.level = 2)
#'
fetch_wnba_team_schedule <- function(team_id, season = NULL, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL
  base_url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/basketball/wnba/teams/%s/schedule", team_id)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("wnba_schedule_raw", data, envir = .GlobalEnv)
      message("Raw wnba schedule data assigned to: wnba_schedule_raw")
    ...
    assign("wnba_team_schedule", schedule_df, envir = .GlobalEnv)

    season_text <- if (!is.null(season)) season else "current"
    message(sprintf("wnba team schedule assigned to: wnba_team_schedule (Team ID: %s, %s season, %d games)",
                    team_id, season_text, nrow(schedule_df)))

    return(invisible(schedule_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch wnba schedule for team %s: %s", team_id, e$message))
  })
}
