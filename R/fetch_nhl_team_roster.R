#' Fetch NHL team roster data using ESPN Site API
#'
#' @param team_id Character or Numeric. ESPN team ID (e.g., "4" for Boston Bruins).
#' @param raw Logical. If TRUE, assigns raw JSON to global environment as 'nhl_roster_raw'. Default FALSE.
#' @return Invisibly returns the data, but main purpose is global environment assignment.
#' @export
#'
#' @examples
#' # Get roster using team ID
#' fetch_nhl_team_roster(team_id = "4")
#'
#' # Get raw data for custom processing
#' fetch_nhl_team_roster(team_id = "4", raw = TRUE)
#' str(nhl_roster_raw, max.level = 2)
#'
fetch_nhl_team_roster <- function(team_id, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL for NHL
  url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/teams/%s/roster", team_id)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("nhl_roster_raw", data, envir = .GlobalEnv)
      message("Raw NHL roster data assigned to: nhl_roster_raw")
      return(invisible(data))
    }

    # Create clean roster dataset
    roster_df <- create_clean_nhl_roster_dataset(data, team_id)

    # Assign to global environment
    assign("nhl_roster", roster_df, envir = .GlobalEnv)
    message(sprintf("NHL roster data assigned to: nhl_roster (Team ID: %s, %d players)",
                    team_id, nrow(roster_df)))

    return(invisible(roster_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch NHL roster data for team %s: %s", team_id, e$message))
  })
}
...
create_clean_nhl_roster_dataset <- function(data, team_id) {
  ...
  # Process athletes from data$athletes - organized by position groups (Goalies, Defensemen, Forwards)
  ...
  # Process each athlete group (goalies, defensemen, forwards)
  for (group_idx in seq_along(athletes_groups)) {
    group <- athletes_groups[[group_idx]]

    # Get group position (Goalies, Defensemen, Forwards)
    ...
    if ("items" %in% names(group) && length(group[["items"]]) > 0) {
      ...
      for (i in seq_along(athletes)) {
        ...
        # Hockey-specific: Shooting and catching hand
        shoots <- catches <- NA_character_
        if ("shoots" %in% names(athlete_data)) {
          shoots_info <- athlete_data[["shoots"]]
          shoots <- if ("abbreviation" %in% names(shoots_info)) shoots_info[["abbreviation"]] else NA_character_
        }
        if ("catches" %in% names(athlete_data)) {
          catches_info <- athlete_data[["catches"]]
          catches <- if ("abbreviation" %in% names(catches_info)) catches_info[["abbreviation"]] else NA_character_
        }
        ...
        if ("salary" %in% names(athlete_data)) {
          salary <- as.character(athlete_data[["salary"]])
        }
  ...
  if (nrow(result_df) > 0) {
    ...
    # Define position order for NHL
    position_order <- c("G", "D", "C", "LW", "RW", "F")
...
#' Get NHL team IDs dynamically from ESPN API
#'
...
#'
#' @examples
#' # Get all NHL team IDs
#' team_ids <- get_nhl_team_ids()
#' print(team_ids)
#'
#' # Force fresh API call without caching
#' team_ids <- get_nhl_team_ids(cache = FALSE)
#'
get_nhl_team_ids <- function(cache = TRUE) {
  # Check if cached data exists and caching is enabled
  if (cache && exists(".nhl_teams_cache", envir = .GlobalEnv)) {
    return(get(".nhl_teams_cache", envir = .GlobalEnv))
  }

  # ESPN API endpoint for NHL teams
  ...
  tryCatch({
    ...
    if (cache) {
      assign(".nhl_teams_cache", result_df, envir = .GlobalEnv)
    }

    return(result_df)

  }, error = function(e) {
    stop(sprintf("Failed to fetch NHL teams data: %s", e$message))
  })
}

#' Find NHL team ID by name or abbreviation
#'
...
#' find_nhl_team_id("Boston Bruins")
#'
#' # Find team by abbreviation
#' find_nhl_team_id("BOS")
#'
#' # Find teams with partial match
#' find_nhl_team_id("New York")
#'
find_nhl_team_id <- function(team, cache = TRUE) {
  if (missing(team) || is.null(team) || team == "") {
    stop("'team' parameter is required and cannot be empty")
  }

  # Get all teams
  teams_df <- get_nhl_team_ids(cache = cache)
