#' Fetch mens_college_basketball team schedule data using Site API
#'
#' @param team_id Character or Numeric. ESPN team ID (e.g., "12" for LA Clippers).
#' @param season Numeric. Season year (e.g., 2023, 2024). Optional - defaults to current season.
#' @param raw Logical. If TRUE, assigns raw JSON to global environment as 'mens_college_basketball_schedule_raw'. Default FALSE.
...
#' fetch_mens_college_basketball_team_schedule(team_id = "12")
#' head(mens_college_basketball_team_schedule)
#'
#' # Get raw data for custom processing
#' fetch_mens_college_basketball_team_schedule(team_id = "12", season = 2023, raw = TRUE)
#' str(mens_college_basketball_schedule_raw, max.level = 2)
#'
fetch_mens_college_basketball_team_schedule <- function(team_id, season = NULL, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL
  base_url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/teams/%s/schedule", team_id)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("mens_college_basketball_schedule_raw", data, envir = .GlobalEnv)
      message("Raw mens_college_basketball schedule data assigned to: mens_college_basketball_schedule_raw")
      return(invisible(data))
    }

    # Create clean schedule dataset
    schedule_df <- create_clean_team_schedule_dataset(data, team_id, season)

    # Assign to global environment
    assign("mens_college_basketball_team_schedule", schedule_df, envir = .GlobalEnv)

    season_text <- if (!is.null(season)) season else "current"
    message(sprintf("mens_college_basketball team schedule assigned to: mens_college_basketball_team_schedule (Team ID: %s, %s season, %d games)",
                    team_id, season_text, nrow(schedule_df)))

    return(invisible(schedule_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch mens_college_basketball schedule for team %s: %s", team_id, e$message))
  })
}
...
create_clean_team_schedule_dataset <- function(data, team_id, season) {
  ...
  for (i in seq_along(events)) {
    ...
    week_number <- if ("number" %in% names(week_info)) as.integer(week_info[["number"]]) else NA_integer_
  ...
  # Clean up row names and sort by week
  if (nrow(result_df) > 0) {
    rownames(result_df) <- NULL
    # Sort by season type, then week number
    result_df <- result_df[order(result_df$season_type, result_df$week_number, na.last = TRUE), ]
    rownames(result_df) <- NULL
  }
