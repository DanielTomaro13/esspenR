#' Fetch NFL team schedule data using Site API
#'
...
#' @param raw Logical. If TRUE, assigns raw JSON to global environment as 'nfl_schedule_raw'. Default FALSE.
#' @return Invisibly returns the data, but main purpose is global environment assignment.
#' @export
#'
#' @examples
#' # Get current season schedule for Kansas City Chiefs
#' fetch_nfl_team_schedule(team_id = "12")
#' head(nfl_team_schedule)
#'
#' # Get 2023 schedule for Buffalo Bills
#' fetch_nfl_team_schedule(team_id = 2, season = 2023)
#'
#' # Get raw data for custom processing
#' fetch_nfl_team_schedule(team_id = "12", season = 2023, raw = TRUE)
#' str(nfl_schedule_raw, max.level = 2)
#'
fetch_nfl_team_schedule <- function(team_id, season = NULL, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL
  base_url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/football/nfl/teams/%s/schedule", team_id)
  ...
  tryCatch({
    ...
    assign("nfl_team_schedule", schedule_df, envir = .GlobalEnv)

    season_text <- if (!is.null(season)) season else "current"
    message(sprintf("NFL team schedule assigned to: nfl_team_schedule (Team ID: %s, %s season, %d games)",
                    team_id, season_text, nrow(schedule_df)))

    return(invisible(schedule_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch NFL schedule for team %s: %s", team_id, e$message))
  })
}
