#' Safe nested data extraction helper function for wnba boxscore data
#'
...
#' Fetch detailed wnba boxscore using ESPN API
#'
#' Retrieves comprehensive boxscore information for a specific wnba game from ESPN's API.
...
#'   (e.g., "401768050" for a specific wnba game).
#' @param raw Logical. If TRUE, assigns raw JSON response to global environment
#'   as 'wnba_boxscore_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns a list containing the datasets. The main purpose is global
#'   environment assignment of:
#'   \itemize{
#'     \item \code{wnba_player_stats}: Individual player statistics for both teams
#'     \item \code{wnba_team_stats}: Team-level statistics and totals
#'   }
#'
#' @details
...
#' fetch_wnba_boxscore("401768050")
#'
#' # Access player statistics
#' head(wnba_player_stats)
#'
#' # Access team statistics
#' head(wnba_team_stats)
#' }
#'
#' @importFrom httr GET status_code content timeout
#' @export
fetch_wnba_boxscore <- function(event_id, raw = FALSE) {
  # Input validation
  ...
  url <- paste0("https://site.api.espn.com/apis/site/v2/sports/basketball/wnba/summary?event=", event_id)
  ...
  if (isTRUE(raw)) {
    assign("wnba_boxscore_raw", data, envir = .GlobalEnv)
  }
  ...
  assign("wnba_player_stats", player_stats, envir = .GlobalEnv)
  ...
  # Assign to global environment
  assign("wnba_team_stats", team_stats, envir = .GlobalEnv)
