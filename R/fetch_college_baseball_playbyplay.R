#' Safe nested data extraction helper function for college_baseball play-by-play
#'
...
#' @keywords internal
extract_nested_college_baseball_pbp <- function(data, path, default = NA) {
  tryCatch({
    result <- data
    for (key in path) {
...
#' Create college_baseball play-by-play data frames from API response
#'
#' Processes raw JSON response from ESPN college_baseball API into structured data frames
#' containing detailed play-by-play information
#'
#' @param data Raw JSON response from ESPN college_baseball API play-by-play endpoint
...
#' @keywords internal
create_college_baseball_playbyplay_datasets <- function(data, event_id) {
  ...
  header <- extract_nested_college_baseball_pbp(data, c("header"), list())
  season_info <- extract_nested_college_baseball_pbp(header, c("season"), list())
  season_year <- extract_nested_college_baseball_pbp(season_info, c("year"), NA_character_)
  season_type <- extract_nested_college_baseball_pbp(season_info, c("type"), NA_character_)
  venue_name <- attendance <- NA_character_

  # Extract team information from competition
  competitions <- extract_nested_college_baseball_pbp(header, c("competitions"), list())
  ...
  if (length(competitions) > 0) {
    ...
    game_date <- extract_nested_college_baseball_pbp(competition, c("date"), NA_character_)

    # Venue information
    venue <- extract_nested_college_baseball_pbp(competition, c("venue"), list())
    venue_name <- extract_nested_college_baseball_pbp(venue, c("fullName"), NA_character_)
    attendance <- extract_nested_college_baseball_pbp(competition, c("attendance"), NA_character_)

    # Status
    status <- extract_nested_college_baseball_pbp(competition, c("status"), list())
    status_type <- extract_nested_college_baseball_pbp(status, c("type"), list())
    game_status <- extract_nested_college_baseball_pbp(status_type, c("name"), NA_character_)

    # Teams
    competitors <- extract_nested_college_baseball_pbp(competition, c("competitors"), list())
    for (competitor in competitors) {
      team_info <- extract_nested_college_baseball_pbp(competitor, c("team"), list())
      team_id <- extract_nested_college_baseball_pbp(team_info, c("id"), NA_character_)
      team_name <- extract_nested_college_baseball_pbp(team_info, c("displayName"), NA_character_)
      team_score <- extract_nested_college_baseball_pbp(competitor, c("score"), NA_character_)
      home_away <- extract_nested_college_baseball_pbp(competitor, c("homeAway"), NA_character_)
  ...
  plays_df <- extract_nested_college_baseball_pbp(data, c("plays"), data.frame())
...
#' Fetch college_baseball game play-by-play data using ESPN API
#'
#' Retrieves detailed play-by-play information from ESPN's college_baseball API.
#' The function fetches comprehensive play data for a specific college_baseball game.
#'
...
#'   as 'college_baseball_pbp_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns the processed data. The main purpose is global
#'   environment assignment based on return_type:
#'   \itemize{
#'     \item \code{college_baseball_pbp_game_summary} - Game summary data frame
#'     \item \code{college_baseball_pbp_inning_summary} - Inning summary data frame
#'     \item \code{college_baseball_pbp_plays} - Complete plays data frame with all ESPN fields
#'     \item \code{college_baseball_pbp_raw} - Raw JSON response (if raw = TRUE)
#'   }
#'
#' @details
#' The function creates structured data frames with comprehensive play-by-play information:
#'
#' **Game Summary** (\code{college_baseball_pbp_game_summary}):
#' \itemize{
...
#' }
#'
#' **Inning Summary** (\code{college_baseball_pbp_inning_summary}):
#' \itemize{
...
#' }
#'
#' **Plays** (\code{college_baseball_pbp_plays}):
#' \itemize{
...
#' \itemize{
#'   \item \code{college_baseball_pbp_plays$pitchVelocity} - Pitch speeds
#'   \item \code{college_baseball_pbp_plays$pitchType$text} - Pitch type names
#'   \item \code{college_baseball_pbp_plays$participants} - Batter/pitcher details
#'   \item \code{college_baseball_pbp_plays$onSecond$athlete$id} - Runner on second
#'   \item \code{college_baseball_pbp_plays$pitchCoordinate} - Pitch location (x,y)
#' }
#'
#' @examples
#' \dontrun{
#' # Get complete play-by-play for a specific game
#' fetch_college_baseball_game_playbyplay(event_id = "401581132")
#'
#' # Check what data was created
#' head(college_baseball_pbp_game_summary)
#' head(college_baseball_pbp_inning_summary)
#' head(college_baseball_pbp_plays)
#'
#' # Access nested data structures
#' college_baseball_pbp_plays$period$number          # Inning numbers
#' college_baseball_pbp_plays$pitchType$text         # Pitch type names
#' college_baseball_pbp_plays$pitchCoordinate$x      # Pitch location X
#'
#' # Get only plays data
#' fetch_college_baseball_game_playbyplay(event_id = "401581132", return_type = "plays")
#'
#' # Analyze four-seam fastballs over 95 mph
#' fastballs <- college_baseball_pbp_plays[
#'   college_baseball_pbp_plays$pitchType$text == "Four-seam FB" &
#'   college_baseball_pbp_plays$pitchVelocity > 95 &
#'   !is.na(college_baseball_pbp_plays$pitchVelocity),
#' ]
#'
#' # Find all scoring plays
#' scoring_plays <- college_baseball_pbp_plays[
#'   college_baseball_pbp_plays$scoringPlay == TRUE,
#'   c("text", "homeScore", "awayScore", "scoreValue")
#' ]
#'
#' # Analyze pitch sequences in specific counts
#' two_strike_pitches <- college_baseball_pbp_plays[
#'   college_baseball_pbp_plays$pitchCount$strikes == 2 &
#'   !is.na(college_baseball_pbp_plays$pitchType$text),
#' ]
#'
#' # View baserunner situations
#' risp_situations <- college_baseball_pbp_plays[
#'   !is.na(college_baseball_pbp_plays$onSecond$athlete$id) |
#'   !is.na(college_baseball_pbp_plays$onThird$athlete$id),
#' ]
#'
#' # Hit coordinate analysis
#' hits_with_coords <- college_baseball_pbp_plays[
#'   !is.na(college_baseball_pbp_plays$hitCoordinate$x) &
#'   !is.na(college_baseball_pbp_plays$hitCoordinate$y),
#' ]
#'
#' # Get raw data for debugging
#' fetch_college_baseball_game_playbyplay(event_id = "401581132", raw = TRUE)
#' }
#'
#' @importFrom httr GET status_code content timeout
#' @importFrom jsonlite fromJSON
#' @export
fetch_college_baseball_game_playbyplay <- function(event_id, return_type = "all", raw = FALSE) {
  # Input validation
  if (missing(event_id)) {
  ...
  # Build API URL for ESPN college_baseball play-by-play
  url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/baseball/college-baseball/summary?event=%s", event_id)

  message(sprintf("Fetching college_baseball play-by-play for game %s...", event_id))
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("college_baseball_pbp_raw", data, envir = .GlobalEnv)
      message("Raw college_baseball play-by-play data assigned to: college_baseball_pbp_raw")
    ...
    # Create datasets
    datasets <- create_college_baseball_playbyplay_datasets(data, event_id)

    # Assign data based on return_type
    result_data <- list()

    if (return_type %in% c("game_summary", "all")) {
      assign("college_baseball_pbp_game_summary", datasets$game_summary, envir = .GlobalEnv)
      result_data$game_summary <- datasets$game_summary
      message(sprintf("college_baseball play-by-play game summary assigned to: college_baseball_pbp_game_summary (%d records)",
                      nrow(datasets$game_summary)))
    }

    if (return_type %in% c("inning_summary", "all")) {
      assign("college_baseball_pbp_inning_summary", datasets$inning_summary, envir = .GlobalEnv)
      result_data$inning_summary <- datasets$inning_summary
      message(sprintf("college_baseball play-by-play inning summary assigned to: college_baseball_pbp_inning_summary (%d half-innings)",
                      nrow(datasets$inning_summary)))
    }

    if (return_type %in% c("plays", "all")) {
      assign("college_baseball_pbp_plays", datasets$plays, envir = .GlobalEnv)
      result_data$plays <- datasets$plays
      message(sprintf("college_baseball play-by-play plays assigned to: college_baseball_pbp_plays (%d plays)",
                      nrow(datasets$plays)))
    ...
    stop(sprintf("Failed to fetch college_baseball play-by-play for event %s: %s",
                 event_id, e$message))
  })
}

#' Fetch multiple college_baseball game play-by-play data
#'
#' Retrieves play-by-play data for multiple college_baseball games with rate limiting to
#' be respectful to ESPN's API. This function calls \code{\link{fetch_college_baseball_game_playbyplay}}
...
#' @param return_type Character. Type of data to return (default: "all").
#'   Same options as \code{\link{fetch_college_baseball_game_playbyplay}}.
...
#' fetch_multiple_college_baseball_game_playbyplay(game_ids)
#'
#' # Get only plays data for multiple games
#' fetch_multiple_college_baseball_game_playbyplay(game_ids, return_type = "plays")
#'
#' # Use longer delay for larger requests
#' fetch_multiple_college_baseball_game_playbyplay(game_ids, delay = 0.5)
#'
#' # Analyze combined results
#' unique_games <- unique(college_baseball_pbp_game_summary$event_id)
#' cat("Retrieved play-by-play for", length(unique_games), "games\n")
#'
#' # Home run analysis across multiple games
#' home_runs <- college_baseball_pbp_plays[
#'   grepl("home run", college_baseball_pbp_plays$text, ignore.case = TRUE),
#'   c("event_id", "text", "pitchVelocity", "pitchType")
#' ]
#'
#' # Clutch hitting analysis (runners in scoring position, 2 outs)
#' clutch_situations <- college_baseball_pbp_plays[
#'   college_baseball_pbp_plays$outs == 2 &
#'   (!is.na(college_baseball_pbp_plays$onSecond$athlete$id) |
#'    !is.na(college_baseball_pbp_plays$onThird$athlete$id)),
#' ]
#'
#' # Pitching velocity trends
#' pitch_velo <- college_baseball_pbp_plays[
#'   !is.na(college_baseball_pbp_plays$pitchVelocity) &
#'   !is.na(college_baseball_pbp_plays$pitchType$text),
...
#' }
#'
#' @seealso \code{\link{fetch_college_baseball_game_playbyplay}} for single game data
#' @importFrom httr GET status_code content timeout
#' @export
fetch_multiple_college_baseball_game_playbyplay <- function(event_ids, return_type = "all",
                                               delay = 0.3, raw = FALSE) {
  # Input validation
  ...
  message(sprintf("Starting to fetch college_baseball play-by-play data for %d games...", length(event_ids)))

  # Process each game sequentially
  for (i in seq_along(event_ids)) {
    event_id <- event_ids[i]
    message(sprintf("Fetching college_baseball play-by-play for event %s (%d/%d)...", event_id, i, length(event_ids)))

    tryCatch({
      # Fetch individual game data
      game_data <- fetch_college_baseball_game_playbyplay(
        event_id = event_id,
      ...
      if (return_type %in% c("game_summary", "all")) {
        summary_df <- get("college_baseball_pbp_game_summary", envir = .GlobalEnv)
        all_game_summary <- rbind(all_game_summary, summary_df)
      }

      if (return_type %in% c("inning_summary", "all")) {
        inning_df <- get("college_baseball_pbp_inning_summary", envir = .GlobalEnv)
        all_inning_summary <- rbind(all_inning_summary, inning_df)
      }

      if (return_type %in% c("plays", "all")) {
        plays_df <- get("college_baseball_pbp_plays", envir = .GlobalEnv)
        all_plays <- rbind(all_plays, plays_df)
      }

    }, error = function(e) {
      message(sprintf("Failed to fetch college_baseball play-by-play for event %s: %s", event_id, e$message))
    })
  ...
  if (return_type %in% c("game_summary", "all") && nrow(all_game_summary) > 0) {
    ...
    assign("college_baseball_pbp_game_summary", all_game_summary, envir = .GlobalEnv)
    result_data$game_summary <- all_game_summary
    message(sprintf("Combined college_baseball play-by-play game summary assigned to: college_baseball_pbp_game_summary (%d games)",
  ...
  if (return_type %in% c("inning_summary", "all") && nrow(all_inning_summary) > 0) {
    ...
    assign("college_baseball_pbp_inning_summary", all_inning_summary, envir = .GlobalEnv)
    result_data$inning_summary <- all_inning_summary
    unique_games <- length(unique(all_inning_summary$event_id))
    message(sprintf("Combined college_baseball play-by-play inning summary assigned to: college_baseball_pbp_inning_summary (%d games, %d half-innings)",
  ...
  if (return_type %in% c("plays", "all") && nrow(all_plays) > 0) {
    ...
    assign("college_baseball_pbp_plays", all_plays, envir = .GlobalEnv)
    result_data$plays <- all_plays
    unique_games <- length(unique(all_plays$event_id))
    total_plays <- nrow(all_plays)
    message(sprintf("Combined college_baseball play-by-play plays assigned to: college_baseball_pbp_plays (%d games, %d plays)",
