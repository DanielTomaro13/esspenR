#' Safe nested data extraction helper function for NBA play-by-play
#'
...
#' @keywords internal
extract_nested_nba_pbp <- function(data, path, default = NA) {
  tryCatch({
    result <- data
    for (key in path) {
...
#' Create NBA play-by-play data frames from API response
#'
#' Processes raw JSON response from ESPN NBA API into structured data frames
#' containing detailed play-by-play information
#'
#' @param data Raw JSON response from ESPN NBA API play-by-play endpoint
...
#' @keywords internal
create_nba_playbyplay_datasets <- function(data, event_id) {
  ...
  header <- extract_nested_nba_pbp(data, c("header"), list())

  # Extract basic game info
  game_id <- extract_nested_nba_pbp(header, c("id"), event_id)
  season_info <- extract_nested_nba_pbp(header, c("season"), list())
  season_year <- extract_nested_nba_pbp(season_info, c("year"), NA_character_)
  season_type <- extract_nested_nba_pbp(season_info, c("type"), NA_character_)
  week <- extract_nested_nba_pbp(header, c("week"), NA_character_)
  venue_name <- NA_character_

  # Extract team information from competition
  competitions <- extract_nested_nba_pbp(header, c("competitions"), list())
  ...
  if (length(competitions) > 0) {
    ...
    game_date <- extract_nested_nba_pbp(competition, c("date"), NA_character_)

    # Venue information
    venue <- extract_nested_nba_pbp(competition, c("venue"), list())
    venue_name <- extract_nested_nba_pbp(venue, c("fullName"), NA_character_)

    # Status
    status <- extract_nested_nba_pbp(competition, c("status"), list())
    status_type <- extract_nested_nba_pbp(status, c("type"), list())
    game_status <- extract_nested_nba_pbp(status_type, c("name"), NA_character_)

    # Teams
    competitors <- extract_nested_nba_pbp(competition, c("competitors"), list())
    for (competitor in competitors) {
      team_info <- extract_nested_nba_pbp(competitor, c("team"), list())
      team_id <- extract_nested_nba_pbp(team_info, c("id"), NA_character_)
      team_name <- extract_nested_nba_pbp(team_info, c("displayName"), NA_character_)
      team_score <- extract_nested_nba_pbp(competitor, c("score"), NA_character_)
      home_away <- extract_nested_nba_pbp(competitor, c("homeAway"), NA_character_)
  ...
  # Extract plays data
  plays_data <- extract_nested_nba_pbp(data, c("plays"), list())
  total_plays <- 0
  ...
  if (length(plays_data) > 0 && is.data.frame(plays_data)) {
    for (i in seq_len(nrow(plays_data))) {
      ...
      play_id <- extract_nested_nba_pbp(play_data, c("id"), as.character(i))
      sequence_number <- extract_nested_nba_pbp(play_data, c("sequenceNumber"), as.character(i))

      # Period information
      period_info <- extract_nested_nba_pbp(play_data, c("period"), list())
      period <- extract_nested_nba_pbp(period_info, c("number"), NA_character_)
      period_display_name <- extract_nested_nba_pbp(period_info, c("displayName"), NA_character_)

      # Clock information
      clock_info <- extract_nested_nba_pbp(play_data, c("clock"), list())
      clock <- extract_nested_nba_pbp(clock_info, c("displayValue"), NA_character_)

      # Play details
      play_type_info <- extract_nested_nba_pbp(play_data, c("type"), list())
      play_type <- extract_nested_nba_pbp(play_type_info, c("text"), NA_character_)
      play_text <- extract_nested_nba_pbp(play_data, c("text"), NA_character_)

      # Scoring
      score_value <- extract_nested_nba_pbp(play_data, c("scoreValue"), "0")

      # Current score
      home_score <- extract_nested_nba_pbp(play_data, c("homeScore"), NA_character_)
      away_score <- extract_nested_nba_pbp(play_data, c("awayScore"), NA_character_)

      # Shooting play indicator
      shooting_play <- extract_nested_nba_pbp(play_data, c("shootingPlay"), "false")

      # Coordinates for shooting plays
      coordinate_x <- extract_nested_nba_pbp(play_data, c("coordinate", "x"), NA_character_)
      coordinate_y <- extract_nested_nba_pbp(play_data, c("coordinate", "y"), NA_character_)

      # Play team
      play_team <- extract_nested_nba_pbp(play_data, c("team"), list())
      play_team_id <- extract_nested_nba_pbp(play_team, c("id"), NA_character_)
      play_team_abbreviation <- extract_nested_nba_pbp(play_team, c("abbreviation"), NA_character_)

      # Participants (players involved)
      participants_data <- extract_nested_nba_pbp(play_data, c("participants"), list())
      ...
      if (length(participants_data) > 0 && is.data.frame(participants_data)) {
        ...
        for (j in seq_len(nrow(participants_data))) {
          ...
          athlete <- extract_nested_nba_pbp(participant, c("athlete"), list())
          if (length(athlete) > 0) {
            name <- extract_nested_nba_pbp(athlete, c("displayName"), NA_character_)
            if (!is.na(name)) {
...
#' Fetch NBA game play-by-play data using ESPN API
#'
#' Retrieves detailed play-by-play information from ESPN's NBA API.
#' The function fetches comprehensive play data for a specific NBA game.
#'
...
#'   as 'nba_pbp_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns the processed data. The main purpose is global
#'   environment assignment based on return_type:
#'   \itemize{
#'     \item \code{nba_pbp_game_summary} - Game summary data frame
#'     \item \code{nba_pbp_period_summary} - Period summary data frame
#'     \item \code{nba_pbp_plays} - Plays data frame
#'     \item \code{nba_pbp_raw} - Raw JSON response (if raw = TRUE)
#'   }
#'
#' @details
#' The function creates structured data frames with comprehensive play-by-play information:
#'
#' **Game Summary** (\code{nba_pbp_game_summary}):
#' \itemize{
...
#' }
#'
#' **Period Summary** (\code{nba_pbp_period_summary}):
#' \itemize{
...
#' fetch_nba_game_playbyplay(event_id = "401656359")
#'
#' # Check what data was created
#' head(nba_pbp_game_summary)
#' head(nba_pbp_period_summary)
#' head(nba_pbp_plays)
#'
#' # Get only plays data
#' fetch_nba_game_playbyplay(event_id = "401656359", return_type = "plays")
#'
#' # Analyze scoring plays
#' scoring_plays <- nba_pbp_plays[
#'   as.numeric(nba_pbp_plays$score_value) > 0,
#'   c("period", "clock", "team_abbreviation", "play_text", "score_value")
#' ]
#' print(scoring_plays)
#'
#' # View shooting plays with coordinates
#' shooting_plays <- nba_pbp_plays[
#'   nba_pbp_plays$shooting_play == "true",
#'   c("team_abbreviation", "play_text", "coordinate_x", "coordinate_y")
#' ]
#' head(shooting_plays)
#'
#' # Fourth quarter plays
#' fourth_quarter <- nba_pbp_plays[
#'   nba_pbp_plays$period == "4",
#'   c("clock", "team_abbreviation", "play_type", "play_text")
#' ]
#' head(fourth_quarter)
#'
#' # Get raw data for debugging
#' fetch_nba_game_playbyplay(event_id = "401656359", raw = TRUE)
#' }
#'
#' @importFrom httr GET status_code content timeout
#' @importFrom jsonlite fromJSON
#' @export
fetch_nba_game_playbyplay <- function(event_id, return_type = "all", raw = FALSE) {
  # Input validation
  if (missing(event_id)) {
  ...
  # Build API URL for ESPN NBA play-by-play
  url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/basketball/nba/summary?event=%s", event_id)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("nba_pbp_raw", data, envir = .GlobalEnv)
      message("Raw NBA play-by-play data assigned to: nba_pbp_raw")
    ...
    # Create datasets
    datasets <- create_nba_playbyplay_datasets(data, event_id)

    # Assign data based on return_type
    result_data <- list()

    if (return_type %in% c("game_summary", "all")) {
      assign("nba_pbp_game_summary", datasets$game_summary, envir = .GlobalEnv)
      result_data$game_summary <- datasets$game_summary
      message(sprintf("NBA play-by-play game summary assigned to: nba_pbp_game_summary (%d records)",
                      nrow(datasets$game_summary)))
    }

    if (return_type %in% c("period_summary", "all")) {
      assign("nba_pbp_period_summary", datasets$period_summary, envir = .GlobalEnv)
      result_data$period_summary <- datasets$period_summary
      message(sprintf("NBA play-by-play period summary assigned to: nba_pbp_period_summary (%d periods)",
                      nrow(datasets$period_summary)))
    }

    if (return_type %in% c("plays", "all")) {
      assign("nba_pbp_plays", datasets$plays, envir = .GlobalEnv)
      result_data$plays <- datasets$plays
      message(sprintf("NBA play-by-play plays assigned to: nba_pbp_plays (%d plays)",
                      nrow(datasets$plays)))
    }
    ...
    stop(sprintf("Failed to fetch NBA play-by-play for event %s: %s",
                 event_id, e$message))
  })
}

#' Fetch multiple NBA game play-by-play data
#'
...
#' @importFrom httr GET status_code content timeout
#' @export
fetch_multiple_nba_game_playbyplay <- function(event_ids, return_type = "all",
                                               delay = 0.2, raw = FALSE) {
  # Input validation
  ...
  message(sprintf("Starting to fetch NBA play-by-play data for %d games...", length(event_ids)))

  # Process each game sequentially
  for (i in seq_along(event_ids)) {
    event_id <- event_ids[i]
    message(sprintf("Fetching NBA play-by-play for event %s (%d/%d)...", event_id, i, length(event_ids)))

    tryCatch({
      # Fetch individual game data
      game_data <- fetch_nba_game_playbyplay(
        event_id = event_id,
      ...
      if (return_type %in% c("game_summary", "all")) {
        summary_df <- get("nba_pbp_game_summary", envir = .GlobalEnv)
        all_game_summary <- rbind(all_game_summary, summary_df)
      }

      if (return_type %in% c("period_summary", "all")) {
        period_df <- get("nba_pbp_period_summary", envir = .GlobalEnv)
        all_period_summary <- rbind(all_period_summary, period_df)
      }

      if (return_type %in% c("plays", "all")) {
        plays_df <- get("nba_pbp_plays", envir = .GlobalEnv)
        all_plays <- rbind(all_plays, plays_df)
      }

    }, error = function(e) {
      message(sprintf("Failed to fetch NBA play-by-play for event %s: %s", event_id, e$message))
    })
  ...
  if (return_type %in% c("game_summary", "all") && nrow(all_game_summary) > 0) {
    ...
    assign("nba_pbp_game_summary", all_game_summary, envir = .GlobalEnv)
    result_data$game_summary <- all_game_summary
    message(sprintf("Combined NBA play-by-play game summary assigned to: nba_pbp_game_summary (%d games)",
  ...
  if (return_type %in% c("period_summary", "all") && nrow(all_period_summary) > 0) {
    ...
    assign("nba_pbp_period_summary", all_period_summary, envir = .GlobalEnv)
    result_data$period_summary <- all_period_summary
    unique_games <- length(unique(all_period_summary$event_id))
    message(sprintf("Combined NBA play-by-play period summary assigned to: nba_pbp_period_summary (%d games, %d periods)",
  ...
  if (return_type %in% c("plays", "all") && nrow(all_plays) > 0) {
    ...
    assign("nba_pbp_plays", all_plays, envir = .GlobalEnv)
    result_data$plays <- all_plays
    unique_games <- length(unique(all_plays$event_id))
    total_plays <- nrow(all_plays)
    message(sprintf("Combined NBA play-by-play plays assigned to: nba_pbp_plays (%d games, %d plays)",
