#' Safe nested data extraction helper function for NBA boxscore data
#'
...
#' Fetch detailed NBA boxscore using ESPN API
#'
#' Retrieves comprehensive boxscore information for a specific NBA game from ESPN's API.
...
#'   (e.g., "401768050" for a specific NBA game).
#' @param raw Logical. If TRUE, assigns raw JSON response to global environment
#'   as 'nba_boxscore_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns a list containing the datasets. The main purpose is global
#'   environment assignment of:
#'   \itemize{
#'     \item \code{nba_player_stats}: Individual player statistics for both teams
#'     \item \code{nba_team_stats}: Team-level statistics and totals
#'   }
#'
#' @details
...
#' @export
fetch_nba_boxscore <- function(event_id, raw = FALSE) {
  # Input validation
  ...
  url <- paste0("https://site.api.espn.com/apis/site/v2/sports/basketball/nba/summary?event=", event_id)
  ...
  if (isTRUE(raw)) {
    assign("nba_boxscore_raw", data, envir = .GlobalEnv)
  }
  ...
  assign("nba_player_stats", player_stats, envir = .GlobalEnv)
  ...
  # Assign to global environment
  assign("nba_team_stats", team_stats, envir = .GlobalEnv)
