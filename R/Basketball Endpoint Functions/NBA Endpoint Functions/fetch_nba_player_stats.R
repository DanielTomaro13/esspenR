#' Safe nested data extraction helper function for NBA player statistics
#'
...
fetch_player_info <- function(player_id) {
  ...
  url <- paste0("https://site.web.api.espn.com/apis/common/v3/sports/basketball/nba/athletes/", player_id)
...
#' @keywords internal
create_nba_season_summary_dataset <- function(data, player_id, player_info) {
...
#' Create detailed NBA player game log data frame from ESPN API
#'
...
#' @keywords internal
create_nba_player_gamelog_dataset <- function(data, player_id, player_info) {
...
#' Fetch detailed NBA player statistics using ESPN Web API
#'
#' Retrieves comprehensive statistics for a specific NBA player from ESPN's Web API.
#' Extracts data directly from the seasonTypes structure - fast and reliable!
#'
#' @param player_id Character or numeric. NBA player ID from ESPN
...
#'   as 'nba_player_stats_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns a list with both datasets. The main purpose is global
#'   environment assignment of:
#'   \itemize{
#'     \item \code{nba_player_season_summary}: Season averages and totals by season type
#'     \item \code{nba_player_gamelog}: Game-by-game statistics organized by category
#'   }
#'
#' @details
...
#' fetch_nba_player_stats("3032977")
#'
#' # Check season summaries
#' print(nba_player_season_summary)
#'
#' # Check game log
#' head(nba_player_gamelog)
#'
#' # View regular season averages
#' reg_season_avg <- nba_player_season_summary[
#'   nba_player_season_summary$season_display_name == "2024-25 Regular Season" &
#'   nba_player_season_summary$stat_type == "avg", ]
#' print(reg_season_avg[, c("points", "total_rebounds", "assists", "field_goal_pct")])
#'
#' # View playoff games
#' playoff_games <- nba_player_gamelog[
#'   nba_player_gamelog$season_display_name == "2024-25 Postseason", ]
#' head(playoff_games[, c("category_display_name", "points", "total_rebounds", "assists")])
#'
#' # Calculate averages from game log
#' all_games <- nba_player_gamelog[!is.na(nba_player_gamelog$points), ]
...
#'
#' @export
fetch_nba_player_stats <- function(player_id, raw = FALSE) {
  ...
  url <- paste0("https://site.web.api.espn.com/apis/common/v3/sports/basketball/nba/athletes/",
                player_id, "/gamelog")
  ...
  if (isTRUE(raw)) {
    assign("nba_player_stats_raw", data, envir = .GlobalEnv)
  }
  ...
  gamelog <- create_nba_player_gamelog_dataset(data, player_id, player_info)
  assign("nba_player_gamelog", gamelog, envir = .GlobalEnv)
