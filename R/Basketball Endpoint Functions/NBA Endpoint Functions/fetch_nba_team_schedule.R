#' Fetch nba team schedule data using Site API
#'
...
#' @param raw Logical. If TRUE, assigns raw JSON to global environment as 'nba_schedule_raw'. Default FALSE.
...
#' fetch_nba_team_schedule(team_id = "12")
#' head(nba_team_schedule)
#'
#' # Get raw data for custom processing
#' fetch_nba_team_schedule(team_id = "12", season = 2023, raw = TRUE)
#' str(nba_schedule_raw, max.level = 2)
#'
fetch_nba_team_schedule <- function(team_id, season = NULL, raw = FALSE) {
  # Validate inputs
  if (missing(team_id)) {
  ...
  # Build URL
  base_url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/basketball/nba/teams/%s/schedule", team_id)
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("nba_schedule_raw", data, envir = .GlobalEnv)
      message("Raw nba schedule data assigned to: nba_schedule_raw")
    ...
    assign("nba_team_schedule", schedule_df, envir = .GlobalEnv)

    season_text <- if (!is.null(season)) season else "current"
    message(sprintf("nba team schedule assigned to: nba_team_schedule (Team ID: %s, %s season, %d games)",
                    team_id, season_text, nrow(schedule_df)))

    return(invisible(schedule_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch nba schedule for team %s: %s", team_id, e$message))
  })
}
