#' Safe nested data extraction helper function for NBA team news
#'
...
#' @keywords internal
extract_nested_nba_news <- function(data, path, default = NA) {
  tryCatch({
    result <- data
    for (key in path) {
...
#' Clean and process text content for NBA news
#'
...
#' @keywords internal
clean_nba_text_content <- function(text) {
...
#' Create NBA team news data frame from Site API response
#'
...
#' @keywords internal
create_nba_news_dataset <- function(data, team_id) {
  ...
  # Extract articles
  articles <- extract_nested_nba_news(data, c("articles"), list())

  for (i in seq_along(articles)) {
    article <- articles[[i]]

    # Basic article information
    article_id <- extract_nested_nba_news(article, c("id"), NA_character_)
    headline <- extract_nested_nba_news(article, c("headline"), NA_character_)
    description <- extract_nested_nba_news(article, c("description"), NA_character_)
    story <- extract_nested_nba_news(article, c("story"), NA_character_)
    published <- extract_nested_nba_news(article, c("published"), NA_character_)
    last_modified <- extract_nested_nba_news(article, c("lastModified"), NA_character_)
    premium <- extract_nested_nba_news(article, c("premium"), "false")
    byline <- extract_nested_nba_news(article, c("byline"), NA_character_)
    data_source_identifier <- extract_nested_nba_news(article, c("dataSourceIdentifier"), NA_character_)
    section <- extract_nested_nba_news(article, c("section"), NA_character_)
    type <- extract_nested_nba_news(article, c("type"), NA_character_)

    # Keywords
    keywords_list <- extract_nested_nba_news(article, c("keywords"), list())
    ...
    links <- extract_nested_nba_news(article, c("links"), list())
    url <- NA_character_
    if (length(links) > 0) {
      web_link <- links[["web"]]
      if (!is.null(web_link)) {
        url <- extract_nested_nba_news(web_link, c("href"), NA_character_)
      }
    }
    ...
    video <- extract_nested_nba_news(article, c("video"), list())
    if (length(video) > 0 && !is.null(video)) {
      video_id <- extract_nested_nba_news(video, c("id"), NA_character_)
      video_headline <- extract_nested_nba_news(video, c("headline"), NA_character_)
      video_caption <- extract_nested_nba_news(video, c("caption"), NA_character_)
      video_description <- extract_nested_nba_news(video, c("description"), NA_character_)
      video_premium <- extract_nested_nba_news(video, c("premium"), "false")
      video_duration <- extract_nested_nba_news(video, c("duration"), NA_character_)
    }
    ...
    images <- extract_nested_nba_news(article, c("images"), list())
    if (length(images) > 0) {
      # Take the first image
      first_image <- images[[1]]
      if (!is.null(first_image)) {
        image_id <- extract_nested_nba_news(first_image, c("id"), NA_character_)
        image_name <- extract_nested_nba_news(first_image, c("name"), NA_character_)
        image_caption <- extract_nested_nba_news(first_image, c("caption"), NA_character_)
        image_url <- extract_nested_nba_news(first_image, c("url"), NA_character_)
        image_width <- extract_nested_nba_news(first_image, c("width"), NA_character_)
        image_height <- extract_nested_nba_news(first_image, c("height"), NA_character_)
      }
    }

    # Clean text content
    headline <- clean_nba_text_content(headline)
    description <- clean_nba_text_content(description)
    story <- clean_nba_text_content(story)
    byline <- clean_nba_text_content(byline)
    video_headline <- clean_nba_text_content(video_headline)
    video_caption <- clean_nba_text_content(video_caption)
    video_description <- clean_nba_text_content(video_description)
    image_caption <- clean_nba_text_content(image_caption)
...
#' Get NBA team ID mapping
#'
#' Returns a mapping of team abbreviations to team IDs for ESPN NBA API
#' @return Named vector with team abbreviations as names and IDs as values
#' @export
get_nba_team_ids <- function() {
  team_ids <- c(
...
#' Fetch NBA team-specific news using Site API
#'
#' Retrieves team-specific news articles from ESPN's NBA Site API.
#' The function fetches comprehensive news information including articles,
#' videos, images, and metadata for a specific NBA team.
#'
...
#'   as 'nba_team_news_raw' for debugging purposes (default: FALSE).
#'
#' @return Invisibly returns the processed data frame. The main purpose is global
#'   environment assignment of \code{nba_team_news} containing:
#'   \itemize{
...
#' Use \code{get_nba_team_ids()} to see all available team mappings.
#'
#' @examples
#' \dontrun{
#' # Get news for Los Angeles Lakers
#' fetch_nba_team_news("LAL")
#'
#' # Get news using team ID
#' fetch_nba_team_news("13")
#'
#' # Get limited number of articles
#' fetch_nba_team_news("BOS", limit = 20)
#'
#' # Check the data
#' head(nba_team_news)
#'
#' # View recent headlines
#' recent_news <- nba_team_news[1:10, c("headline", "published", "byline")]
#' print(recent_news)
#'
#' # Find video content
#' video_news <- nba_team_news[!is.na(nba_team_news$video_id),
#'                            c("headline", "video_headline", "video_duration")]
#' print(video_news)
#'
#' # Get articles with images
#' image_news <- nba_team_news[!is.na(nba_team_news$image_url),
...
#' }
#'
#' @seealso \code{\link{get_nba_team_ids}} for team ID mappings
#' @importFrom httr GET status_code content timeout http_status
#' @importFrom jsonlite fromJSON
#' @export
fetch_nba_team_news <- function(team, limit = 50, raw = FALSE) {
  # Input validation
  ...
  # Build API URL for NBA
  url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/basketball/nba/news?team=%s&limit=%d",
                 team_id, limit)

  message(sprintf("Fetching NBA news for team %s...", team_abbr))
  ...
  tryCatch({
    ...
    if (isTRUE(raw)) {
      assign("nba_team_news_raw", data, envir = .GlobalEnv)
      message("Raw NBA team news data assigned to: nba_team_news_raw")
    ...
    # Create news dataset
    news_df <- create_nba_news_dataset(data, team_id)

    # Assign to global environment
    assign("nba_team_news", news_df, envir = .GlobalEnv)
    ...
    message(sprintf("NBA team news assigned to: nba_team_news (%d articles)", total_articles))
    ...
    stop(sprintf("Failed to fetch NBA team news for team %s: %s", team_abbr, e$message))
  })
}

#' Fetch NBA news for multiple teams
#'
#' Retrieves team-specific news for multiple NBA teams with rate limiting.
#' This function calls \code{\link{fetch_nba_team_news}} for each team
...
#'   environment assignment of combined \code{nba_team_news} from all teams.
#'
#' @details
...
#' fetch_multiple_nba_team_news(c("LAL", "GSW", "DEN", "PHX"))
#'
#' # Get news for specific teams with fewer articles each
#' fetch_multiple_nba_team_news(c("BOS", "MIA", "NYK"), limit = 15)
#'
#' # Use longer delay for larger requests
#' east_teams <- c("BOS", "MIA", "PHI", "MIL", "ATL")
#' fetch_multiple_nba_team_news(east_teams, delay = 1.0)
#'
#' # Check combined results
#' head(nba_team_news)
#' table(nba_team_news$team_id)
#'
#' # Find trade deadline coverage
#' trade_news <- nba_team_news[
#'   grepl("trade|deadline", nba_team_news$headline, ignore.case = TRUE),
#'   c("team_id", "headline", "published")
#' ]
#' print(trade_news)
#' }
#'
#' @seealso \code{\link{fetch_nba_team_news}} for single team data
#' @importFrom httr GET status_code content timeout
#' @export
fetch_multiple_nba_team_news <- function(teams, limit = 25, delay = 0.5, raw = FALSE) {
  # Input validation
  if (length(teams) == 0) {
  ...
  message(sprintf("Starting to fetch NBA team news for %d teams...", length(teams)))

  # Process each team sequentially
  for (i in seq_along(teams)) {
    team <- teams[i]
    message(sprintf("Fetching NBA team news for %s (%d/%d)...", team, i, length(teams)))

    tryCatch({
      # Fetch individual team data
      team_data <- fetch_nba_team_news(
        team = team,
        limit = limit,
        raw = raw
      )
      ...
      # Combine data
      news_df <- get("nba_team_news", envir = .GlobalEnv)
      all_news <- rbind(all_news, news_df)

    }, error = function(e) {
      message(sprintf("Failed to fetch NBA team news for %s: %s", team, e$message))
    })
  ...
  if (nrow(all_news) > 0) {
    ...
    assign("nba_team_news", all_news, envir = .GlobalEnv)

    unique_teams <- length(unique(all_news$team_id))
    total_articles <- nrow(all_news)

    message(sprintf("Combined NBA team news assigned to: nba_team_news (%d teams, %d articles)",
