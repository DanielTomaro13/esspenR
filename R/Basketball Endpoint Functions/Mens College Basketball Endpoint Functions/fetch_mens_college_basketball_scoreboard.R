#' Fetch mens_college_basketball scoreboard data using Site API
#'
...
#'
#' @examples
#' # Get mens_college_basketball scoreboard for current date
#' fetch_mens_college_basketball_scoreboard()
#' head(mens_college_basketball_scoreboard)
#'
#' # Get scoreboard for specific date
#' fetch_mens_college_basketball_scoreboard(dates = "20241225")
#'
#' # Get raw data
#' fetch_mens_college_basketball_scoreboard(raw = TRUE)
#'
fetch_mens_college_basketball_scoreboard <- function(year = NULL, week = NULL, seasontype = 2, dates = NULL, raw = FALSE) {
  ...
  # Fetch and parse
  tryCatch({
    message("Fetching mens_college_basketball scoreboard data...")
    ...
    if (isTRUE(raw)) {
      assign("mens_college_basketball_scoreboard_raw", data, envir = .GlobalEnv)
      message("Raw mens_college_basketball scoreboard data assigned to: mens_college_basketball_scoreboard_raw")
      return(invisible(data))
    }

    # Create clean scoreboard dataset
    scoreboard_df <- create_clean_mens_college_basketball_scoreboard_dataset(data, year, week, seasontype, dates)

    # Assign to global environment
    assign("mens_college_basketball_scoreboard", scoreboard_df, envir = .GlobalEnv)
    message(sprintf("mens_college_basketball scoreboard data assigned to: mens_college_basketball_scoreboard (%d games)", nrow(scoreboard_df)))

    return(invisible(scoreboard_df))

  }, error = function(e) {
    stop(sprintf("Failed to fetch mens_college_basketball scoreboard data: %s", e$message))
  })
}

#' Create clean mens_college_basketball scoreboard dataset from ESPN Site API response
#'
...
create_clean_mens_college_basketball_scoreboard_dataset <- function(data, year, week, seasontype, dates) {
